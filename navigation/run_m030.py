...
M030 Adaptive Confidence-Gated Yaw Architecture Execution Script (run_m030.py)

Evaluates M030 Adaptive Confidence-Gated Yaw Architecture on benchmark datasets.
DO NOT modify original baselines M028 or M029.
"""
...
from navigation.m030_adaptive_yaw import EKF7StateM030
...
def run_m030_benchmark():
    print("=" * 80)
    print("  SIH 2026 PS 26168 — M030 ADAPTIVE CONFIDENCE-GATED YAW ARCHITECTURE")
    print("  SpeedNet v2 (W=40) + Adaptive NIS Yaw Gate + Multi-Anchor EKF")
    print("=" * 80)
    ...
    if not (os.path.exists(s_path) and os.path.exists(v_path)):
        print(f"[ERROR] Benchmark dataset files not found at:\n  {s_path}\n  {v_path}")
        return
    ...
    print("Running SpeedNet v2 (W=40) predictions...", flush=True)
    model = load_speednet_v2_model('models/speednet_v2_w40.pth', window_size=40)
    model.eval()

    train_mean = np.mean(X_raw_all[:int(n_total * 0.70)], axis=0)
    train_std = np.std(X_raw_all[:int(n_total * 0.70)], axis=0)
    ...
    apm = APMModule(jerk_threshold=-1.0, max_correction_mps=0.50)
    ...
    init_heading_rad = np.radians(latched_heading_deg) if is_latched_valid else np.radians(vbox_heading_deg[start_idx])

    init_state = [
        x_gt_all[start_idx - 300], y_gt_all[start_idx - 300],
        vx_gt_all[start_idx - 300], vy_gt_all[start_idx - 300],
        init_heading_rad, 0.0, 0.0
    ]
    ekf = EKF7StateM030(init_state, dt=dt)
    heading_manager = HeadingAnchorManager(dt=dt, v_min_gnss=2.0, n_straight_threshold=10)

    v_est_history = []
    x_est_list, y_est_list = [], []

    sim_start = start_idx - 300
    sim_end = start_idx + 3000
    ...
    for idx in range(sim_start, sim_end):
        ...
        if not is_outage:
            ...
            # M030 Adaptive Confidence-Gated Yaw Rate Update
    ...
    pos_err = np.sqrt((x_est_list[-1] - x_gt_all[sim_end - 1])**2 + (y_est_list[-1] - y_gt_all[sim_end - 1])**2)
    print(f"\nM030 300s Outage Final Position Error: {pos_err:.2f} meters")
    print(f"Adaptive Gate Stats: {ekf.gate_stats}")
