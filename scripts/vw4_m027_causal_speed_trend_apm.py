...
# Causal SpeedNet Derivative Array Pre-computation
# dv_dt[k] = (v_speednet[k] - v_speednet[k-1]) / dt
dv_dt_array = np.zeros(n_total)
for k in range(1, n_total):
    vk   = v_ml_raw_dict.get(k, 0.0)
    vk_1 = v_ml_raw_dict.get(k-1, 0.0)
    dv_dt_array[k] = (vk - vk_1) / dt
...
# ── Navigation Engine for M027 Causal Speed-Trend APM ────────────────────
def run_navigation_m027(
    ...
    for idx in range(sim_start, sim_end):
        ...
        if not is_outage:
            ...
            # M027 Causal Speed-Trend APM Logic
            if (not is_stat_pred) and (len(v_est_history) >= 5):
                ...
                if is_decel and is_turn_ok:
                    ...
                    # Speed-Trend Confirmation Gate
                    is_speed_trend_ok = True
                    if dv_thresh is not None:
                        dv_dt_val = dv_dt_array[idx]
                        is_speed_trend_ok = (dv_dt_val < dv_thresh)

                    if is_speed_trend_ok:
...
def compute_metrics_m027(x_dr, y_dr, v_dr, psi_deg, v_meas_arr, start, dur):
    n = int(dur / dt)
...
xd, yd, vd, pd, _, vm = run_navigation_m027(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=300, dv_thresh=None)
m_f0 = compute_metrics_m027(xd, yd, vd, pd, vm, start_idx, 300)

p60  = compute_metrics_m027(*run_navigation_m027(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=60, dv_thresh=None)[:4], run_navigation_m027(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=60, dv_thresh=None)[5], start_idx, 60)['final_pos_m']
p120 = compute_metrics_m027(*run_navigation_m027(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=120, dv_thresh=None)[:4], run_navigation_m027(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=120, dv_thresh=None)[5], start_idx, 120)['final_pos_m']
...
print("======================================================================")
print("2. VALIDATION SPEED-TREND CONFIRMATION EVALUATION (Partition 88566:107535)")
print("======================================================================")

best_val_err = float('inf')
best_val_thresh = None
best_val_name = 'F0 (Control M019 / No Speed-Trend Gate)'

val_trend_specs = [
    ('F0 (Control M019 / No Gate)', None),
    ('F1 (dv/dt < -0.15 m/s²)', -0.15),
    ('F2 (dv/dt < -0.30 m/s²)', -0.30),
    ('F3 (dv/dt < -0.50 m/s²)', -0.50),
    ('F4 (dv/dt < -0.75 m/s²)', -0.75)
]

for tname, tval in val_trend_specs:
    xd, yd, vd, pd, _, vm = run_navigation_m027(
        v_f4_dict, prob_stat_dict, sim_start_idx=idx_train_end + 300, duration_sec=300,
        dv_thresh=tval
    )
    m_val = compute_metrics_m027(xd, yd, vd, pd, vm, idx_train_end + 300, 300)
    print(f"  Val Candidate: {tname:<32} -> Val 300s Error = {m_val['final_pos_m']:.2f} m (Mean = {m_val['mean_pos_m']:.2f} m)", flush=True)
    if m_val['final_pos_m'] < best_val_err:
        best_val_err = m_val['final_pos_m']
        best_val_thresh = tval
        best_val_name = tname
...
candidate_specs = [
    {
        'id': 'F0 (Control M019)',
        'desc': 'M019 Control Baseline (No Speed-Trend Gate)',
        'kwargs': {'dv_thresh': None}
    },
    {
        'id': 'F1 (dv/dt < -0.15)',
        'desc': 'Speed-Trend Gate dv/dt < -0.15 m/s²',
        'kwargs': {'dv_thresh': -0.15}
    },
    {
        'id': 'F2 (dv/dt < -0.30)',
        'desc': 'Speed-Trend Gate dv/dt < -0.30 m/s²',
        'kwargs': {'dv_thresh': -0.30}
    },
    {
        'id': 'F3 (dv/dt < -0.50)',
        'desc': 'Speed-Trend Gate dv/dt < -0.50 m/s²',
        'kwargs': {'dv_thresh': -0.50}
    },
    {
        'id': 'F4 (dv/dt < -0.75)',
        'desc': 'Speed-Trend Gate dv/dt < -0.75 m/s²',
        'kwargs': {'dv_thresh': -0.75}
    },
    {
        'id': 'F5 (Best Validated + M019)',
        'desc': f"Best Validated Configuration ({best_val_name})",
        'kwargs': {'dv_thresh': best_val_thresh}
    }
]
...
for spec in candidate_specs:
    ...
    for dur in [60, 120, 300]:
        xd, yd, vd, pd, stats, vm = run_navigation_m027(
            v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=dur,
            **spec['kwargs']
        )
        m = compute_metrics_m027(xd, yd, vd, pd, vm, start_idx, dur)
...
# ── 4. APM Activation & Suppression Diagnostics ────────────────────────────
print("\n======================================================================")
print("4. APM ACTIVATION & SPEED-TREND SUPPRESSION DIAGNOSTICS SUMMARY")
print("======================================================================")
...
os.makedirs('plots/vw4/m027_causal_speed_trend_apm', exist_ok=True)
...
plt.title('M027: Speed-Trend Confirmed APM 300s Position Error', fontsize=12, fontweight='bold')
plt.xlabel('Outage Elapsed Time (s)')
plt.ylabel('Position Error (m)')
plt.grid(True, alpha=0.3)
plt.legend(loc='upper left', fontsize=8)
plt.tight_layout()
plt.savefig('plots/vw4/m027_causal_speed_trend_apm/pos_error_vs_time.png', dpi=300)
plt.close()
...
for cid, (xd, yd) in traj_series.items():
    if cid in ['F0 (Control M019)', 'F1 (dv/dt < -0.15)', 'F4 (dv/dt < -0.75)']:
        plt.plot(xd, yd, label=f"{cid} ({nav_err_series[cid][-1]:.1f} m)")

plt.title('M027: 2D Dead-Reckoning Trajectory Comparison', fontsize=12, fontweight='bold')
...
plt.axis('equal')
plt.tight_layout()
plt.savefig('plots/vw4/m027_causal_speed_trend_apm/trajectory_comparison.png', dpi=300)
plt.close()

# Save JSON Summary & NPZ Predictions
summary_data = {
    'milestone': 'M027',
    'title': 'Causal Speed-Trend Confirmation for APM Activation',
    'm019_benchmark': 220.12,
    'selected_val_thresh': best_val_thresh,
    'results_table': results_table
}

with open('results/vw4_m027_causal_speed_trend_apm_summary.json', 'w') as f:
    json.dump(summary_data, f, indent=2)

np.savez(
    'results/vw4_m027_causal_speed_trend_apm_predictions.npz',
    test_indices=test_indices,
    v_gt=vbox_vel_ms[test_indices],
    pos_err_f0=nav_err_series['F0 (Control M019)'],
    pos_err_f1=nav_err_series['F1 (dv/dt < -0.15)'],
    pos_err_f4=nav_err_series['F4 (dv/dt < -0.75)']
)

print("\nSaved M027 summary JSON and predictions NPZ.")
print("M027 execution complete.")
