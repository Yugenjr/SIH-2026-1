...
# ── Navigation Engine for M029 Jerk Threshold Robustness ─────────────────
def run_navigation_m029(
        ...
        if not is_outage:
            ...
            if (not is_stat_pred) and (len(v_est_history) >= 5):
                ...
                if is_decel and is_turn_ok:
                    ...
                    apm_stats['jerk_values'].append(j_val)
...
def compute_metrics_m029(x_dr, y_dr, v_dr, psi_deg, v_meas_arr, start, dur):
    n = int(dur / dt)
...
print("\n======================================================================")
print("1. M028 CONTROL REPRODUCTION VERIFICATION (j_long < -1.00 m/s³)")
print("======================================================================")

xd, yd, vd, pd, _, vm = run_navigation_m029(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=300, jerk_thresh=-1.00)
m_f0 = compute_metrics_m029(xd, yd, vd, pd, vm, start_idx, 300)

p60  = compute_metrics_m029(*run_navigation_m029(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=60, jerk_thresh=-1.00)[:4], run_navigation_m029(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=60, jerk_thresh=-1.00)[5], start_idx, 60)['final_pos_m']
p120 = compute_metrics_m029(*run_navigation_m029(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=120, jerk_thresh=-1.00)[:4], run_navigation_m029(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=120, jerk_thresh=-1.00)[5], start_idx, 120)['final_pos_m']
...
print("======================================================================")
print("2. VALIDATION LOCAL JERK THRESHOLD EVALUATION (Partition 88566:107535)")
print("======================================================================")

best_val_err = float('inf')
best_val_thresh = -1.00
best_val_name = 'F0 (Control -1.00 m/s³)'

val_jerk_local_specs = [
    ('F1 (j_long < -0.75 m/s³)', -0.75),
    ('F2 (j_long < -0.90 m/s³)', -0.90),
    ('F0 (Control -1.00 m/s³)', -1.00),
    ('F3 (j_long < -1.10 m/s³)', -1.10),
    ('F4 (j_long < -1.25 m/s³)', -1.25)
]

for jname, jval in val_jerk_local_specs:
    xd, yd, vd, pd, _, vm = run_navigation_m029(
        v_f4_dict, prob_stat_dict, sim_start_idx=idx_train_end + 300, duration_sec=300,
        jerk_thresh=jval
    )
    m_val = compute_metrics_m029(xd, yd, vd, pd, vm, idx_train_end + 300, 300)
    print(f"  Val Candidate: {jname:<30} -> Val 300s Error = {m_val['final_pos_m']:.2f} m (Mean = {m_val['mean_pos_m']:.2f} m)", flush=True)
    if m_val['final_pos_m'] < best_val_err:
        best_val_err = m_val['final_pos_m']
        best_val_thresh = jval
        best_val_name = jname
...
candidate_specs = [
    {
        'id': 'F0 (Control -1.00 m/s³)',
        'desc': 'M028 Baseline Control (j_long < -1.00 m/s³)',
        'kwargs': {'jerk_thresh': -1.00}
    },
    {
        'id': 'F1 (j_long < -0.75 m/s³)',
        'desc': 'Local Threshold j_long < -0.75 m/s³',
        'kwargs': {'jerk_thresh': -0.75}
    },
    {
        'id': 'F2 (j_long < -0.90 m/s³)',
        'desc': 'Local Threshold j_long < -0.90 m/s³',
        'kwargs': {'jerk_thresh': -0.90}
    },
    {
        'id': 'F3 (j_long < -1.10 m/s³)',
        'desc': 'Local Threshold j_long < -1.10 m/s³',
        'kwargs': {'jerk_thresh': -1.10}
    },
    {
        'id': 'F4 (j_long < -1.25 m/s³)',
        'desc': 'Local Threshold j_long < -1.25 m/s³',
        'kwargs': {'jerk_thresh': -1.25}
    },
    {
        'id': 'F5 (Best Validated + M028)',
        'desc': f"Best Validated Configuration ({best_val_name})",
        'kwargs': {'jerk_thresh': best_val_thresh}
    }
]
...
for spec in candidate_specs:
    ...
    for dur in [60, 120, 300]:
        xd, yd, vd, pd, stats, vm = run_navigation_m029(
            v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=dur,
            **spec['kwargs']
        )
        m = compute_metrics_m029(xd, yd, vd, pd, vm, start_idx, dur)
...
# ── 4. APM Activation & Jerk Threshold Diagnostics ────────────────────────
print("\n======================================================================")
print("4. APM ACTIVATION & LOCAL THRESHOLD DIAGNOSTICS SUMMARY")
print("======================================================================")
...
os.makedirs('plots/vw4/m029_jerk_threshold_robustness', exist_ok=True)
...
plt.title('M029: Jerk Threshold Local Robustness 300s Position Error', fontsize=12, fontweight='bold')
plt.xlabel('Outage Elapsed Time (s)')
plt.ylabel('Position Error (m)')
plt.grid(True, alpha=0.3)
plt.legend(loc='upper left', fontsize=8)
plt.tight_layout()
plt.savefig('plots/vw4/m029_jerk_threshold_robustness/pos_error_vs_time.png', dpi=300)
plt.close()
...
for cid, (xd, yd) in traj_series.items():
    if cid in ['F0 (Control -1.00 m/s³)', 'F1 (j_long < -0.75 m/s³)', 'F4 (j_long < -1.25 m/s³)']:
        plt.plot(xd, yd, label=f"{cid} ({nav_err_series[cid][-1]:.1f} m)")

plt.title('M029: 2D Dead-Reckoning Trajectory Comparison', fontsize=12, fontweight='bold')
...
plt.axis('equal')
plt.tight_layout()
plt.savefig('plots/vw4/m029_jerk_threshold_robustness/trajectory_comparison.png', dpi=300)
plt.close()

# Save JSON Summary & NPZ Predictions
summary_data = {
    'milestone': 'M029',
    'title': 'Jerk-Gate Robustness and Local Threshold Ablation',
    'm028_benchmark': 218.93,
    'selected_val_thresh': best_val_thresh,
    'results_table': results_table
}

with open('results/vw4_m029_jerk_threshold_robustness_summary.json', 'w') as f:
    json.dump(summary_data, f, indent=2)

np.savez(
    'results/vw4_m029_jerk_threshold_robustness_predictions.npz',
    test_indices=test_indices,
    v_gt=vbox_vel_ms[test_indices],
    pos_err_f0=nav_err_series['F0 (Control -1.00 m/s³)'],
    pos_err_f1=nav_err_series['F1 (j_long < -0.75 m/s³)'],
    pos_err_f4=nav_err_series['F4 (j_long < -1.25 m/s³)']
)

print("\nSaved M029 summary JSON and predictions NPZ.")
print("M029 execution complete.")
