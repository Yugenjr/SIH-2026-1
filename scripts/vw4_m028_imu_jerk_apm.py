...
# IMU Longitudinal Acceleration & Causal Jerk Pre-computation
...
# ── Navigation Engine for M028 Causal IMU Jerk APM ────────────────────────
def run_navigation_m028(
...
def compute_metrics_m028(x_dr, y_dr, v_dr, psi_deg, v_meas_arr, start, dur):
    n = int(dur / dt)
...
xd, yd, vd, pd, _, vm = run_navigation_m028(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=300, jerk_thresh=None)
m_f0 = compute_metrics_m028(xd, yd, vd, pd, vm, start_idx, 300)

p60  = compute_metrics_m028(*run_navigation_m028(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=60, jerk_thresh=None)[:4], run_navigation_m028(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=60, jerk_thresh=None)[5], start_idx, 60)['final_pos_m']
p120 = compute_metrics_m028(*run_navigation_m028(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=120, jerk_thresh=None)[:4], run_navigation_m028(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=120, jerk_thresh=None)[5], start_idx, 120)['final_pos_m']
...
print("======================================================================")
print("2. VALIDATION CAUSAL JERK GATING EVALUATION (Partition 88566:107535)")
print("======================================================================")

best_val_err = float('inf')
best_val_thresh = None
best_val_name = 'F0 (Control M019 / No Jerk Gate)'

val_jerk_specs = [
    ('F0 (Control M019 / No Gate)', None),
    ('F1 (j_long < -0.50 m/s³)', -0.50),
    ('F2 (j_long < -1.00 m/s³)', -1.00),
    ('F3 (j_long < -1.50 m/s³)', -1.50),
    ('F4 (j_long < -2.00 m/s³)', -2.00)
]

for jname, jval in val_jerk_specs:
    xd, yd, vd, pd, _, vm = run_navigation_m028(
        v_f4_dict, prob_stat_dict, sim_start_idx=idx_train_end + 300, duration_sec=300,
        jerk_thresh=jval
    )
    m_val = compute_metrics_m028(xd, yd, vd, pd, vm, idx_train_end + 300, 300)
...
candidate_specs = [
    {
        'id': 'F0 (Control M019)',
        'desc': 'M019 Control Baseline (No Jerk Gate)',
        'kwargs': {'jerk_thresh': None}
    },
    {
        'id': 'F1 (j_long < -0.50)',
        'desc': 'Causal Jerk Gate j_long < -0.50 m/s³',
        'kwargs': {'jerk_thresh': -0.50}
    },
    {
        'id': 'F2 (j_long < -1.00)',
        'desc': 'Causal Jerk Gate j_long < -1.00 m/s³',
        'kwargs': {'jerk_thresh': -1.00}
    },
    {
        'id': 'F3 (j_long < -1.50)',
        'desc': 'Causal Jerk Gate j_long < -1.50 m/s³',
        'kwargs': {'jerk_thresh': -1.50}
    },
    {
        'id': 'F4 (j_long < -2.00)',
        'desc': 'Causal Jerk Gate j_long < -2.00 m/s³',
        'kwargs': {'jerk_thresh': -2.00}
    },
    {
...
for spec in candidate_specs:
    ...
    for dur in [60, 120, 300]:
        xd, yd, vd, pd, stats, vm = run_navigation_m028(
            v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=dur,
            **spec['kwargs']
        )
        m = compute_metrics_m028(xd, yd, vd, pd, vm, start_idx, dur)
...
# ── 4. APM Activation & Jerk Suppression Diagnostics ───────────────────────
print("\n======================================================================")
print("4. APM ACTIVATION & JERK SUPPRESSION DIAGNOSTICS SUMMARY")
print("======================================================================")
...
os.makedirs('plots/vw4/m028_imu_jerk_apm', exist_ok=True)
...
plt.title('M028: Causal IMU Jerk Gated APM 300s Position Error', fontsize=12, fontweight='bold')
plt.xlabel('Outage Elapsed Time (s)')
plt.ylabel('Position Error (m)')
plt.grid(True, alpha=0.3)
plt.legend(loc='upper left', fontsize=8)
plt.tight_layout()
plt.savefig('plots/vw4/m028_imu_jerk_apm/pos_error_vs_time.png', dpi=300)
plt.close()
...
for cid, (xd, yd) in traj_series.items():
    if cid in ['F0 (Control M019)', 'F1 (j_long < -0.50)', 'F4 (j_long < -2.00)']:
        plt.plot(xd, yd, label=f"{cid} ({nav_err_series[cid][-1]:.1f} m)")

plt.title('M028: 2D Dead-Reckoning Trajectory Comparison', fontsize=12, fontweight='bold')
...
plt.axis('equal')
plt.tight_layout()
plt.savefig('plots/vw4/m028_imu_jerk_apm/trajectory_comparison.png', dpi=300)
plt.close()

# Save JSON Summary & NPZ Predictions
summary_data = {
    'milestone': 'M028',
    'title': 'Causal IMU Longitudinal-Acceleration Jerk Gating for APM',
    'm019_benchmark': 220.12,
    'selected_val_thresh': best_val_thresh,
    'results_table': results_table
}

with open('results/vw4_m028_imu_jerk_apm_summary.json', 'w') as f:
    json.dump(summary_data, f, indent=2)

np.savez(
    'results/vw4_m028_imu_jerk_apm_predictions.npz',
    test_indices=test_indices,
    v_gt=vbox_vel_ms[test_indices],
    pos_err_f0=nav_err_series['F0 (Control M019)'],
    pos_err_f1=nav_err_series['F1 (j_long < -0.50)'],
    pos_err_f4=nav_err_series['F4 (j_long < -2.00)']
)

print("\nSaved M028 summary JSON and predictions NPZ.")
print("M028 execution complete.")
