# -*- coding: utf-8 -*-
"""
vw4_m011_speednet_v4_temporal_context.py
=========================================
Milestone M011 — Sequence Window / Temporal Context Experiment (SpeedNet v4)

Objective:
  Test whether expanding the temporal sequence context window (W = 40, 50, 60, 80)
  or introducing dilated temporal convolutions (SpeedNet v4 Dilated, d=2) reduces
  systematic speed overestimation during braking/deceleration and beats the 
  provenance-locked SpeedNet v2 + Raw Gyro + Fixed NHC benchmark of 263.11 m @ 300s.

Experimental Variants:
  F0: Control — SpeedNet v2 (W=40, 4.0s sequence window)
  F1: SpeedNet v2 (W=50, 5.0s sequence window)
  F2: SpeedNet v2 (W=60, 6.0s sequence window)
  F3: SpeedNet v2 (W=80, 8.0s sequence window)
  F4: SpeedNet v4 Dilated (W=60, d=2 dilated Conv1d, 12.0s effective receptive field)

Rules:
  - Provenance-locked baseline Case F0 MUST reproduce 263.11 m @ 300s
  - Model selection strictly via Validation Speed MAE (88566:107535)
  - Unseen Test Partition start_idx = 108,000 locked for final evaluation
"""
...
os.makedirs('plots/vw4/m011_speednet_v4', exist_ok=True)
...
class SpeedNetV2(nn.Module):
    def __init__(self, window_size=40, in_channels=6, hidden_dim=64):
        ...
        self.fc_shared = nn.Sequential(nn.Linear(hidden_dim * 2, 64), nn.ReLU())
        self.fc_speed  = nn.Sequential(nn.Linear(64, 32), nn.ReLU(), nn.Linear(32, 1))
        self.fc_yaw    = nn.Sequential(nn.Linear(64, 32), nn.ReLU(), nn.Linear(32, 1))
        self.fc_stat   = nn.Sequential(nn.Linear(64, 32), nn.ReLU(), nn.Linear(32, 1))
        self.fc_delta_v= nn.Sequential(nn.Linear(64, 16), nn.ReLU(), nn.Linear(16, 1))
    ...
    def forward(self, x):
        ...
        feat_shared = self.fc_shared(feat_final)
        v_fwd      = F.relu(self.fc_speed(feat_shared)).squeeze(-1)
...
class SpeedNetV4Dilated(nn.Module):
    """ SpeedNet v4: Dilated Temporal Convolutions (dilation d=2) """
    def __init__(self, window_size=60, in_channels=6, hidden_dim=64, dilation=2):
        super(SpeedNetV4Dilated, self).__init__()
        self.window_size = window_size
        self.conv = nn.Sequential(
            nn.Conv1d(in_channels, 32, kernel_size=3, padding=dilation, dilation=dilation),
            nn.BatchNorm1d(32),
            nn.ReLU(),
            nn.Conv1d(32, 64, kernel_size=3, padding=dilation, dilation=dilation),
            nn.BatchNorm1d(64),
            nn.ReLU()
        )
        self.bilstm = nn.LSTM(64, hidden_dim, num_layers=1, batch_first=True, bidirectional=True)
...
# ── Training Function ────────────────────────────────────────────────────────
def train_and_eval_model(model_cls, window_size, model_name, is_dilated=False, num_epochs=15, batch_size=1024, lr=1e-3):
    ckpt_path = f'models/{model_name}.pth'
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = model_cls(window_size=window_size).to(device)

    # Check if pre-trained checkpoint exists for control F0
    if os.path.exists(ckpt_path) and model_name == 'speednet_v2_w40':
        print(f"Loading existing checkpoint for {model_name} from {ckpt_path}...", flush=True)
        model.load_state_dict(torch.load(ckpt_path, map_location=device))
    else:
        print(f"\n--- Training {model_name} (Window={window_size}) for {num_epochs} Epochs ---", flush=True)
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for x_b, v_b, w_b, stat_b, delta_b in train_loader:
                ...
                loss = (1.0 * criterion_speed(v_pred, v_b) +
                        0.5 * criterion_yaw(w_pred, w_b) +
                        0.5 * criterion_stat(logit_stat, stat_b) +
                        0.2 * criterion_delta(delta_pred, delta_b))
                loss.backward()
                optimizer.step()
            ...
            val_mae = val_speed_err / len(val_dataset)
    ...
    # Evaluate full trajectory predictions
    model.eval()
    sub_windows = np.lib.stride_tricks.sliding_window_view(X_norm_all, window_shape=(window_size, 6), axis=(0, 1)).squeeze(1)
    full_dataset = TensorDataset(torch.tensor(sub_windows, dtype=torch.float32))
...
def evaluate_metrics(x_dr, y_dr, v_dr, psi_deg, v_ml_dict, start, dur):
    n = int(dur / dt)
    ...
    # Pure Speed predictions accuracy on test window
...
# ─────────────────────────────────────────────────────────────────────────────
# EXPERIMENTAL MATRIX SETUP & SELECTION
# ─────────────────────────────────────────────────────────────────────────────
candidates = [
    ("F0: SpeedNet v2 Control (W=40)",   SpeedNetV2,       40, "speednet_v2_w40", False),
    ("F1: SpeedNet v2 (W=50)",           SpeedNetV2,       50, "speednet_v2_w50", False),
    ("F2: SpeedNet v2 (W=60)",           SpeedNetV2,       60, "speednet_v2_w60", False),
    ("F3: SpeedNet v2 (W=80)",           SpeedNetV2,       80, "speednet_v2_w80", False),
    ("F4: SpeedNet v4 Dilated (W=60, d=2)", SpeedNetV4Dilated, 60, "speednet_v4_w60_d2", True),
]

print("\n" + "="*70, flush=True)
print("TRAINING & VALIDATION SELECTION OF M011 TEMPORAL CANDIDATES", flush=True)
print("="*70, flush=True)
...
for idx, (label, cls, w, mname, is_dilated) in enumerate(candidates):
    model, v_ml_dict, prob_stat_dict, num_params = train_and_eval_model(cls, w, mname, is_dilated, num_epochs=15)

    # Compute validation set speed MAE
    ...
    if val_mae < best_val_mae and idx > 0: # Select best candidate among new variants
        best_val_mae = val_mae
        selected_candidate_idx = idx

    candidate_results.append({
        'label': label, 'cls': cls, 'w': w, 'mname': mname,
    ...
    print(f"  {label:<38} | Params: {num_params:>7} | Val Speed MAE: {val_mae:>6.2f} km/h | Val Bias: {val_bias:>+6.2f} km/h", flush=True)
...
eval_summary_table = []
print(f"  {'Variant Name':<42} {'Val MAE':>8} {'Test MAE':>9} {'60s (m)':>8} {'120s (m)':>9} {'300s (m)':>9} {'vs Base':>10}", flush=True)
print("  " + "-"*102, flush=True)

benchmark_300s = 0.0

for idx, c in enumerate(candidate_results):
    row = {'label': c['label'], 'w': c['w'], 'params': c['params'], 'val_mae': c['val_speed_mae_kmh'], 'is_selected': c.get('is_selected', False)}
    ...
    for dur in [60, 120, 300]:
        ...
        m = evaluate_metrics(xd, yd, vd, psi_d, c['v_ml_dict'], start_idx, dur)
        row[f'{dur}s'] = m
    ...
    print(f"  {c['label']:<42} {c['val_speed_mae_kmh']:>7.2f}k {test_mae:>8.2f}k {p60:>8.1f} {p120:>9.1f} {p300:>9.1f} {delta_str:>10}", flush=True)

# Baseline reproduction assertion
print(f"\n  REPRODUCED PROVENANCE BASELINE (F0 Control W=40): {benchmark_300s:.2f} m @ 300s (Target = 263.11 m)", flush=True)
...
# ─────────────────────────────────────────────────────────────────────────────
print("\n" + "="*70, flush=True)
print("BRAKING & DECELERATION FOCUSED DIAGNOSTIC (Unseen Test Partition)", flush=True)
print("="*70, flush=True)
...
regime_speed_report = []
print(f"  {'Regime':<18} | {'F0 (W=40) Bias':>15} | {'F2 (W=60) Bias':>15} | {'F4 (Dilated) Bias':>18}", flush=True)
print("  " + "-"*75, flush=True)

f0_preds = np.array(eval_summary_table[0]['300s']['v_preds_arr'])
f2_preds = np.array(eval_summary_table[2]['300s']['v_preds_arr'])
f4_preds = np.array(eval_summary_table[4]['300s']['v_preds_arr'])
...
for rid, rname in enumerate(REGIME_NAMES):
    ...
    b4 = round(float(np.mean(f4_preds[mask] - test_vgt_kmh[mask])), 2)
    regime_speed_report.append({'regime': rname, 'f0_bias': b0, 'f2_bias': b2, 'f4_bias': b4})
    print(f"  {rname:<18} | {b0:>+14.2f}k | {b2:>+14.2f}k | {b4:>+17.2f}k", flush=True)

# ─────────────────────────────────────────────────────────────────────────────
# M011 VERDICT & DECISION
# ─────────────────────────────────────────────────────────────────────────────
BENCHMARK_TARGET = 263.11

if selected_300s_pos < BENCHMARK_TARGET - 2.0:
    m011_verdict = "SUCCESS"
...
print(f"\n" + "="*70, flush=True)
print(f"M011 VERDICT: {m011_verdict}", flush=True)
...
# PLOTS GENERATION
# ─────────────────────────────────────────────────────────────────────────────
print("\nGenerating M011 diagnostic plots...", flush=True)
P_DIR = 'plots/vw4/m011_speednet_v4'
t300  = np.arange(n300) * dt

# Plot 1: Receptive Field vs Validation & Test Speed MAE
fig, ax = plt.subplots(figsize=(8, 5))
w_vals  = [r['w'] for r in eval_summary_table]
val_maes = [r['val_mae'] for r in eval_summary_table]
test_maes = [r['300s']['speed_mae_kmh'] for r in eval_summary_table]
labels_p1 = [r['label'][:20] for r in eval_summary_table]
x_p1 = np.arange(len(w_vals))
...
ax.set_title('M011 — Temporal Context Window vs Speed Estimation MAE', fontweight='bold')
ax.legend(); ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(f'{P_DIR}/receptive_field_vs_speed_mae.png', dpi=150); plt.close()

# Plot 2: Speed Prediction vs GT during Braking/Deceleration
...
ax.plot(t300[braking_mask], f0_preds[braking_mask], 'r.', label='F0 (W=40 Control)', alpha=0.5)
ax.plot(t300[braking_mask], f2_preds[braking_mask], 'b.', label='F2 (W=60 Window)', alpha=0.5)
ax.plot(t300[braking_mask], f4_preds[braking_mask], 'g.', label='F4 (Dilated W=60 d=2)', alpha=0.5)
ax.set_xlabel('Time in Outage (s)'); ax.set_ylabel('Speed (km/h)')
ax.set_title('M011 — Speed Estimation Comparison During Braking Regimes', fontweight='bold')
ax.legend(); ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(f'{P_DIR}/braking_speed_prediction.png', dpi=150); plt.close()

# Plot 3: Speed Bias by Maneuver Regime
...
b4_vals   = [r['f4_bias'] for r in regime_speed_report]
x3 = np.arange(len(r_labels3))
w3 = 0.25
ax.bar(x3 - w3, b0_vals, w3, label='F0 (W=40 Control)', color='#4CAF50')
ax.bar(x3, b2_vals, w3, label='F2 (W=60)', color='#2196F3')
ax.bar(x3 + w3, b4_vals, w3, label='F4 (Dilated W=60 d=2)', color='#9C27B0')
ax.set_xticks(x3); ax.set_xticklabels(r_labels3, rotation=30, ha='right')
ax.axhline(0, color='black', linewidth=1)
ax.set_ylabel('Speed Bias (km/h)'); ax.set_title('M011 — Speed Prediction Bias Across Maneuver Regimes', fontweight='bold')
ax.legend(); ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(f'{P_DIR}/speed_bias_by_regime.png', dpi=150); plt.close()

# Plot 4: 300s Navigation Position Error vs Time
fig, ax = plt.subplots(figsize=(12, 6))
colors_p4 = ['#4CAF50', '#2196F3', '#FF9800', '#9C27B0', '#E91E63']
for i, r in enumerate(eval_summary_table):
    lbl = f"{r['label']} ({r['300s']['final_pos_m']:.1f}m)"
...
ax.set_title('M011 — 300s Dead-Reckoning Position Error Growth Across Receptive Fields', fontweight='bold')
ax.legend(); ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(f'{P_DIR}/pos_error_vs_time.png', dpi=150); plt.close()

# Plot 5: Multi-Horizon Comparison Bar Chart
fig, ax = plt.subplots(figsize=(10, 5))
...
ax.set_ylabel('Position Error (m)'); ax.set_title('M011 — Navigation Position Error Matrix Across Outage Horizons', fontweight='bold')
ax.legend(); ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(f'{P_DIR}/multi_horizon_comparison.png', dpi=150); plt.close()

# Plot 6: Speed MAE vs Navigation Error Scatter Plot
...
ax.set_title('M011 — Scientific Check: Speed Prediction MAE vs Integrated Navigation Error', fontweight='bold')
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(f'{P_DIR}/speed_mae_vs_nav_error.png', dpi=150); plt.close()

print(f"  M011 diagnostic plots saved to {P_DIR}/", flush=True)
...
summary_data = {
    'milestone': 'M011',
    'experiment': 'Sequence Window / Temporal Context Experiment (SpeedNet v4)',
    'dataset': 'Vw04',
    'test_start': start_idx,
    'provenance_benchmark_300s_m': BENCHMARK_TARGET,
    'verdict': m011_verdict,
...
with open('results/vw4_m011_speednet_v4_summary.json', 'w') as f:
    json.dump(summary_data, f, indent=2)

np.savez('results/vw4_m011_speednet_v4_predictions.npz',
         f0_preds=f0_preds, f2_preds=f2_preds, f4_preds=f4_preds,
         test_vgt_kmh=test_vgt_kmh, test_regimes=test_regimes)

print("M011 summary JSON and predictions NPZ saved.", flush=True)
print("M011 execution complete.", flush=True)
