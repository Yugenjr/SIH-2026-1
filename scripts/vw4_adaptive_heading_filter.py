...
os.makedirs('plots/vw4/adaptive_heading', exist_ok=True)
...
a_lat_raw = raw_ax                   # m/s^2 (raw lateral accel)
...
w_yaw_imu_raw = -gyro_pitch          # rad/s
...
# Pre-outage gyro bias estimation (Strictly from pre-outage window [start_idx - 100 : start_idx])
b_gyro_static = float(np.mean(w_yaw_imu_raw[start_idx - 100 : start_idx]))

# --- 6 Adaptive Heading Estimation Variants ---

def run_variant_a(start_idx, duration_sec):
...
def run_variant_b(start_idx, duration_sec):
    n = int(duration_sec / dt)
    v_dr = np.array([v_ml_dict[start_idx + k] for k in range(n)])
    w_dr = w_yaw_imu_raw[start_idx : start_idx + n] - b_gyro_static
    psi_dr = np.zeros(n)
    x_dr = np.zeros(n)
    y_dr = np.zeros(n)
    psi_dr[0] = np.radians(vbox_heading_deg[start_idx])
...
def run_variant_c(start_idx, duration_sec):
    n = int(duration_sec / dt)
    v_dr = np.array([v_ml_dict[start_idx + k] for k in range(n)])
    w_ml = np.array([w_ml_dict[start_idx + k] for k in range(n)])
    w_imu = w_yaw_imu_raw[start_idx : start_idx + n] - b_gyro_static
    w_dr = 0.5 * w_ml + 0.5 * w_imu
...
def run_variant_d(start_idx, duration_sec):
    n = int(duration_sec / dt)
    v_dr = np.array([v_ml_dict[start_idx + k] for k in range(n)])
    w_ml = np.array([w_ml_dict[start_idx + k] for k in range(n)])
    w_imu = w_yaw_imu_raw[start_idx : start_idx + n] - b_gyro_static
    ...
    t0 = time.perf_counter()
    for k in range(n):
        innov = abs(w_ml[k] - w_imu[k])
        alpha = 0.5 * np.exp(-innov / 0.05)
        w_dr[k] = alpha * w_ml[k] + (1.0 - alpha) * w_imu[k]
...
def run_variant_e(start_idx, duration_sec):
    n = int(duration_sec / dt)
    v_dr = np.array([v_ml_dict[start_idx + k] for k in range(n)])
    w_ml = np.array([w_ml_dict[start_idx + k] for k in range(n)])
    w_imu_raw = w_yaw_imu_raw[start_idx : start_idx + n]
    
    b_bias = b_gyro_static
    w_dr = np.zeros(n)
    psi_dr = np.zeros(n)
    x_dr = np.zeros(n)
    y_dr = np.zeros(n)
    ...
    for k in range(n):
        ...
        if abs(w_ml[k]) < np.radians(0.5) and v_dr[k] > 1.0 and imu_accel_std < 0.2:
            meas_bias = w_imu_raw[k]
            b_bias = 0.98 * b_bias + 0.02 * meas_bias
            
        w_dr[k] = w_imu_raw[k] - b_bias
...
def run_variant_f(start_idx, duration_sec):
    n = int(duration_sec / dt)
    v_dr = np.array([v_ml_dict[start_idx + k] for k in range(n)])
    w_ml = np.array([w_ml_dict[start_idx + k] for k in range(n)])
    w_imu_raw = w_yaw_imu_raw[start_idx : start_idx + n]
    a_lat_tc = a_lat_tilt_comp[start_idx : start_idx + n]
    ...
    for k in range(n):
        ...
        if imu_accel_std < 0.15 and np.linalg.norm(X_raw_all[idx, 3:6]) < 0.02:
            v_curr = 0.0
            w_curr = 0.0
            b_bias = 0.95 * b_bias + 0.05 * w_imu_raw[k]
        else:
            w_imu_cal = w_imu_raw[k] - b_bias
            innov = abs(w_ml[k] - w_imu_cal)
            alpha = 0.5 * np.exp(-innov / 0.05)
            w_fused = alpha * w_ml[k] + (1.0 - alpha) * w_imu_cal
            
            if abs(w_ml[k]) < np.radians(0.5) and v_curr > 1.0:
                w_curr = 0.0
                b_bias = 0.99 * b_bias + 0.01 * w_imu_raw[k]
            elif v_curr > 2.0 and abs(a_lat_tc[k]) > 0.1:
                w_centrifugal = a_lat_tc[k] / v_curr
                w_curr = 0.70 * w_fused + 0.30 * w_centrifugal
            else:
                w_curr = w_fused
...
def run_raw_dr(start_idx, duration_sec):
    n = int(duration_sec / dt)
    v_dr = np.zeros(n); psi_dr = np.zeros(n); x_dr = np.zeros(n); y_dr = np.zeros(n)
    v_dr[0] = vbox_vel_ms[start_idx]; psi_dr[0] = np.radians(vbox_heading_deg[start_idx])
    t0 = time.perf_counter()
    for i in range(1, n):
...
def evaluate_dr(x_dr, y_dr, v_dr, psi_dr_deg, w_dr_rads, start_idx, duration_sec, case_name):
    ...
    h_growth_rate_degs = float(final_h_err_deg / duration_sec)
    ...
    return {
        ...
        'h_growth_rate_degs': round(h_growth_rate_degs, 4),
        'x_dr': x_dr,
        'y_dr': y_dr,
        'v_dr': v_dr,
        'w_dr': w_dr_rads,
...
ablation_variants = [
    ("A. ML Yaw Only (W=30 Baseline)", run_variant_a),
    ("B. IMU Gyro Yaw Only", run_variant_b),
    ("C. ML Yaw + Fixed Fusion", run_variant_c),
    ("D. ML Yaw + Adaptive Innovation Fusion", run_variant_d),
    ("E. Adaptive Yaw-Bias Filter", run_variant_e),
    ("F. Final Adaptive Heading Filter", run_variant_f)
]
...
for dur in [60, 120, 300]:
    plot_data_abl[dur] = []
    for var_name, var_func in ablation_variants:
        x_dr, y_dr, v_dr, psi_dr_deg, w_dr, lat_ms = var_func(start_idx, dur)
        res = evaluate_dr(x_dr, y_dr, v_dr, psi_dr_deg, w_dr, start_idx, dur, var_name)
        res['latency_ms'] = round(lat_ms, 4)
        plot_data_abl[dur].append(res)
        all_ablation_results.append(res)

# Benchmark Baseline Models for Final Table
print("Computing baselines for final benchmark table...")
benchmark_matrix = []
for dur in [60, 120, 300]:
    ...
    res_m30 = next(r for r in plot_data_abl[dur] if r['case_name'] == "A. ML Yaw Only (W=30 Baseline)")
    res_f = next(r for r in plot_data_abl[dur] if r['case_name'] == "F. Final Adaptive Heading Filter")
    ...
    # Position Error Reduction Attributable to Heading (comparing Variant F vs Case E ML Speed + True Heading floor of 562.48m)
    err_red_heading_pct = float(((res_m30['final_pos_err_m'] - res_f['final_pos_err_m']) / max(1e-3, res_m30['final_pos_err_m'] - 562.48)) * 100.0)
    
    res_f_copy = {k: v for k, v in res_f.items() if not isinstance(v, np.ndarray)}
    res_f_copy['pct_imp_ml_w30'] = round(pct_imp_ml_w30, 2)
    res_f_copy['pct_imp_ekf'] = round(pct_imp_ekf, 2)
    res_f_copy['err_red_heading_pct'] = round(max(0.0, min(100.0, err_red_heading_pct)), 2)
    
    benchmark_matrix.extend([
        {k: v for k, v in res_r.items() if not isinstance(v, np.ndarray)},
        {k: v for k, v in res_c.items() if not isinstance(v, np.ndarray)},
        {k: v for k, v in res_e.items() if not isinstance(v, np.ndarray)},
        {k: v for k, v in res_m30.items() if not isinstance(v, np.ndarray)},
        res_f_copy
    ])
...
with open('results/vw4_adaptive_heading_results.json', 'w') as f:
    json.dump(json_output, f, indent=2)

df_abl = pd.DataFrame(json_output['ablation_results'])[['duration_sec', 'case_name', 'cde_pct', 'final_pos_err_m', 'max_pos_err_m', 'drift_rate_ms', 'v_mae_kmh', 'final_h_err_deg', 'max_h_err_deg', 'h_growth_rate_degs', 'latency_ms']]

print("\n" + "="*145)
print("VW4 UNSEEN TEST SET: 6-VARIANT ADAPTIVE HEADING ESTIMATION ABLATION MATRIX")
print("="*145)
print(df_abl.to_string(index=False))
print("="*145)

df_bench = pd.DataFrame(json_output['benchmark_matrix'])[['duration_sec', 'case_name', 'cde_pct', 'final_pos_err_m', 'max_pos_err_m', 'drift_rate_ms', 'v_mae_kmh', 'final_h_err_deg']]
print("\n" + "="*125)
...
var_colors = {
    "A. ML Yaw Only (W=30 Baseline)": "#9b59b6",
    "B. IMU Gyro Yaw Only": "#e74c3c",
    "C. ML Yaw + Fixed Fusion": "#3498db",
    "D. ML Yaw + Adaptive Innovation Fusion": "#e67e22",
    "E. Adaptive Yaw-Bias Filter": "#16a085",
    "F. Final Adaptive Heading Filter": "#27ae60"
}

var_styles = {
    "A. ML Yaw Only (W=30 Baseline)": ":",
    "B. IMU Gyro Yaw Only": "--",
    "C. ML Yaw + Fixed Fusion": "-.",
    "D. ML Yaw + Adaptive Innovation Fusion": "-.",
    "E. Adaptive Yaw-Bias Filter": "-",
    "F. Final Adaptive Heading Filter": "-"
}

# Plots 1, 2, 3: Trajectory Comparisons for 60s, 120s, 300s
for dur in [60, 120, 300]:
    runs = plot_data_abl[dur]
    fig, ax = plt.subplots(figsize=(10, 8))
    ax.plot(runs[0]['x_gt'], runs[0]['y_gt'], 'k-', linewidth=3.0, label='VBOX Ground Truth')
    for r in runs:
        c_name = r['case_name']
        ax.plot(r['x_dr'], r['y_dr'], color=var_colors[c_name], linestyle=var_styles[c_name], linewidth=1.8, label=c_name)
    ...
    plt.tight_layout()
    plt.savefig(f'plots/vw4/adaptive_heading/outage_{dur}s_trajectory.png', dpi=300)
    plt.close()

# Plot 4: 300s Position Error Growth
fig, ax = plt.subplots(figsize=(10, 6))
runs300 = plot_data_abl[300]
t_arr300 = np.arange(3000) * dt
for r in runs300:
    c_name = r['case_name']
    ax.plot(t_arr300, r['pos_err'], color=var_colors[c_name], linestyle=var_styles[c_name], linewidth=2.0, label=c_name)
...
ax.legend(fontsize=8, loc='best')
plt.tight_layout()
plt.savefig('plots/vw4/adaptive_heading/outage_300s_position_error.png', dpi=300)
plt.close()

# Plot 5: 300s Heading Error Growth
fig, ax = plt.subplots(figsize=(10, 6))
for r in runs300:
    c_name = r['case_name']
    ax.plot(t_arr300, r['h_err_arr'], color=var_colors[c_name], linestyle=var_styles[c_name], linewidth=2.0, label=c_name)
...
ax.legend(fontsize=8, loc='best')
plt.tight_layout()
plt.savefig('plots/vw4/adaptive_heading/outage_300s_heading_error.png', dpi=300)
plt.close()

# Plot 6: Heading Rate Comparison (ML vs IMU vs Fused vs Ground Truth)
fig, ax = plt.subplots(figsize=(11, 5))
w_gt_degs = np.degrees(runs300[0]['w_gt'])
w_ml_degs = np.degrees(runs300[0]['w_dr'])
w_imu_degs = np.degrees(runs300[1]['w_dr'])
w_fused_degs = np.degrees(runs300[5]['w_dr'])

ax.plot(t_arr300[:600], w_gt_degs[:600], 'k-', linewidth=2.5, label='VBOX Ground Truth Yaw Rate')
ax.plot(t_arr300[:600], w_ml_degs[:600], color=var_colors["A. ML Yaw Only (W=30 Baseline)"], linestyle=':', linewidth=1.5, label='ML Yaw Rate')
ax.plot(t_arr300[:600], w_imu_degs[:600], color=var_colors["B. IMU Gyro Yaw Only"], linestyle='--', linewidth=1.5, label='IMU Gyro Yaw Rate')
ax.plot(t_arr300[:600], w_fused_degs[:600], color=var_colors["F. Final Adaptive Heading Filter"], linestyle='-', linewidth=1.8, label='Variant F Fused Yaw Rate')
ax.set_title('Vw4 Unseen Test Set: Yaw Rate Signals Comparison (First 60s)', fontsize=14, fontweight='bold')
ax.set_xlabel('Outage Time (seconds)')
ax.set_ylabel('Yaw Rate (deg/s)')
ax.grid(True, linestyle='--', alpha=0.5)
ax.legend(fontsize=8, loc='best')
plt.tight_layout()
plt.savefig('plots/vw4/adaptive_heading/heading_rate_comparison.png', dpi=300)
plt.close()

# Plot 7: Ablation Comparison Bar Chart
fig, axs = plt.subplots(1, 2, figsize=(15, 6))
fig.suptitle('Adaptive Heading Estimation 6-Variant Ablation Summary', fontsize=14, fontweight='bold')
durations = [60, 120, 300]
x_indices = np.arange(len(durations))
width = 0.13

for idx, (v_name, _) in enumerate(ablation_variants):
    pos_errs = [next(r['final_pos_err_m'] for r in plot_data_abl[d] if r['case_name'] == v_name) for d in durations]
    cde_vals = [next(r['cde_pct'] for r in plot_data_abl[d] if r['case_name'] == v_name) for d in durations]
    axs[0].bar(x_indices + idx * width, pos_errs, width, label=v_name, color=var_colors[v_name])
    axs[1].bar(x_indices + idx * width, cde_vals, width, label=v_name, color=var_colors[v_name])
...
plt.savefig('plots/vw4/adaptive_heading/ablation_comparison.png', dpi=300)
plt.close()

# Plot 8: Final Benchmark Comparison Bar Chart
fig, ax = plt.subplots(figsize=(10, 6))
bench_names = [b['case_name'] for b in benchmark_matrix if b['duration_sec'] == 300]
bench_pos_300 = [b['final_pos_err_m'] for b in benchmark_matrix if b['duration_sec'] == 300]
colors_bench = ['#95a5a6', '#7f8c8d', '#e74c3c', '#3498db', '#27ae60']

ax.bar(np.arange(len(bench_names)), bench_pos_300, color=colors_bench, width=0.55)
ax.set_title('300s GNSS Outage Position Error Comparison Across All Systems', fontsize=14, fontweight='bold')
ax.set_xticks(np.arange(len(bench_names)))
ax.set_xticklabels(bench_names, rotation=15, ha='right', fontsize=9)
ax.set_ylabel('Final Position Error at 300s (meters)')
ax.grid(True, linestyle='--', alpha=0.5)

for i, v in enumerate(bench_pos_300):
    ax.text(i, v + 50, f"{v:.1f} m", ha='center', fontweight='bold', fontsize=9)

plt.tight_layout()
summary_plot_file = 'plots/vw4/adaptive_heading/final_benchmark_comparison.png'
