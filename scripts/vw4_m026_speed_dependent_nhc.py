...
# ── Base NHC Parameters Audit ──────────────────────────────────────────────
# R0 = 0.20^2 = 0.04 m^2/s^2
# State vector: [x, y, vx, vy, psi, ba, bw]
# Lateral velocity in ENU frame: v_lat = -vx * cos(psi) + vy * sin(psi)
# H_nhc = [0, 0, -cos(psi), sin(psi), vx*sin(psi) + vy*cos(psi), 0, 0]
# R_nhc(v) = R0 * (1 + gamma * v_est^2), capped at R_max = 5.0 * R0 (0.20 m^2/s^2)
R0_nhc = 0.20**2
R_max_nhc = 5.0 * R0_nhc # 0.20 m^2/s^2 safety cap

# ── Navigation Engine for M026 Speed-Dependent NHC Covariance ────────────
def run_navigation_m026(
    ...
    for idx in range(sim_start, sim_end):
        ...
        if not is_outage:
            ...
            # M026 Speed-Dependent NHC Covariance Logic
            v_curr_est = np.sqrt(x_state[2]**2 + x_state[3]**2) # in m/s
            R_nhc_dynamic = R0_nhc * (1.0 + gamma_val * (v_curr_est**2))
            R_nhc_dynamic = min(R_nhc_dynamic, R_max_nhc)
            ...
            y_nhc = 0.0 - v_lat
            S_nhc = float(H_nhc @ P @ H_nhc.T + R_nhc_dynamic)
            K_nhc = (P @ H_nhc.T) / S_nhc
            x_state = x_state + K_nhc * y_nhc
            P = (np.eye(7) - np.outer(K_nhc, H_nhc)) @ P

            nhc_stats['r_nhc_history'].append(R_nhc_dynamic)
            nhc_stats['y_nhc_history'].append(y_nhc)
            nhc_stats['v_lat_history'].append(v_lat)
    ...
    return x_dr, y_dr, v_dr, psi_deg, nhc_stats, np.array(v_meas_history)

def compute_metrics_m026(x_dr, y_dr, v_dr, psi_deg, v_meas_arr, start, dur):
    n = int(dur / dt)
...
xd, yd, vd, pd, _, vm = run_navigation_m026(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=300, gamma_val=0.0)
m_f0 = compute_metrics_m026(xd, yd, vd, pd, vm, start_idx, 300)

p60  = compute_metrics_m026(*run_navigation_m026(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=60, gamma_val=0.0)[:4], run_navigation_m026(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=60, gamma_val=0.0)[5], start_idx, 60)['final_pos_m']
p120 = compute_metrics_m026(*run_navigation_m026(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=120, gamma_val=0.0)[:4], run_navigation_m026(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=120, gamma_val=0.0)[5], start_idx, 120)['final_pos_m']
...
print("======================================================================")
print("2. VALIDATION SPEED-DEPENDENT NHC EVALUATION (Partition 88566:107535)")
print("======================================================================")
...
val_gamma_specs = [
    ('F0 (Control gamma=0.0)', 0.0),
    ('F1 (gamma=0.005)', 0.005),
    ('F2 (gamma=0.010)', 0.010),
    ('F3 (gamma=0.020)', 0.020),
    ('F4 (gamma=0.050)', 0.050)
]

for gname, gval in val_gamma_specs:
    xd, yd, vd, pd, _, vm = run_navigation_m026(
        v_f4_dict, prob_stat_dict, sim_start_idx=idx_train_end + 300, duration_sec=300,
        gamma_val=gval
    )
    m_val = compute_metrics_m026(xd, yd, vd, pd, vm, idx_train_end + 300, 300)
    print(f"  Val Candidate: {gname:<26} -> Val 300s Error = {m_val['final_pos_m']:.2f} m (Mean = {m_val['mean_pos_m']:.2f} m)", flush=True)
    if m_val['final_pos_m'] < best_val_err:
        best_val_err = m_val['final_pos_m']
        best_val_gamma = gval

print(f"  Selected Validation Best Gamma: gamma = {best_val_gamma:.4f} (Val Error = {best_val_err:.2f} m)\n", flush=True)

# ── 3. Candidate Sweep Execution on Unseen Test set (`start_idx = 108,000`) ──
candidate_specs = [
    {
        'id': 'F0 (Control gamma=0.0)',
        'desc': 'M019 Baseline Control (Fixed R0 = 0.04 m²/s²)',
        'kwargs': {'gamma_val': 0.0}
    },
    {
        'id': 'F1 (gamma=0.005)',
        'desc': 'Speed-Dependent NHC Covariance (gamma = 0.005)',
        'kwargs': {'gamma_val': 0.005}
    },
    {
        'id': 'F2 (gamma=0.010)',
        'desc': 'Speed-Dependent NHC Covariance (gamma = 0.010)',
        'kwargs': {'gamma_val': 0.010}
    },
    {
        'id': 'F3 (gamma=0.020)',
        'desc': 'Speed-Dependent NHC Covariance (gamma = 0.020)',
        'kwargs': {'gamma_val': 0.020}
    },
    {
        'id': 'F4 (gamma=0.050)',
        'desc': 'Speed-Dependent NHC Covariance (gamma = 0.050)',
        'kwargs': {'gamma_val': 0.050}
    },
    {
        'id': 'F5 (Best Validated + M019)',
        'desc': f"Best Validated Configuration (gamma = {best_val_gamma:.4f})",
        'kwargs': {'gamma_val': best_val_gamma}
    }
]
...
for spec in candidate_specs:
    ...
    for dur in [60, 120, 300]:
        xd, yd, vd, pd, stats, vm = run_navigation_m026(
            v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=dur,
            **spec['kwargs']
        )
        m = compute_metrics_m026(xd, yd, vd, pd, vm, start_idx, dur)
    ...
    nhc_diagnostics_dict[cid] = stats_300
...
# ── 4. NHC Covariance Diagnostic Summary ───────────────────────────────────
print("\n======================================================================")
print("4. NHC COVARIANCE & LATERAL VELOCITY DIAGNOSTICS SUMMARY")
print("======================================================================")
print(f"{'Candidate':<26} | {'Mean R_nhc (m²/s²)':<20} | {'Max R_nhc (m²/s²)':<18} | {'Mean |y_nhc| (m/s)'}")
print("-" * 80)
for cid, stats in nhc_diagnostics_dict.items():
    r_hist = stats['r_nhc_history']
    y_hist = stats['y_nhc_history']
    m_r = float(np.mean(r_hist)) if len(r_hist) > 0 else 0.04
    mx_r = float(np.max(r_hist)) if len(r_hist) > 0 else 0.04
    m_y = float(np.mean(np.abs(y_hist))) if len(y_hist) > 0 else 0.0
    print(f"{cid:<26} | {m_r:>20.4f} | {mx_r:>18.4f} | {m_y:>16.4f}")

# ── Save Diagnostic Plots ─────────────────────────────────────────────────
os.makedirs('plots/vw4/m026_speed_dependent_nhc', exist_ok=True)
...
plt.title('M026: Speed-Dependent NHC Covariance 300s Position Error', fontsize=12, fontweight='bold')
plt.xlabel('Outage Elapsed Time (s)')
plt.ylabel('Position Error (m)')
plt.grid(True, alpha=0.3)
plt.legend(loc='upper left', fontsize=8)
plt.tight_layout()
plt.savefig('plots/vw4/m026_speed_dependent_nhc/pos_error_vs_time.png', dpi=300)
plt.close()
...
for cid, (xd, yd) in traj_series.items():
    if cid in ['F0 (Control gamma=0.0)', 'F1 (gamma=0.005)', 'F4 (gamma=0.050)']:
        plt.plot(xd, yd, label=f"{cid} ({nav_err_series[cid][-1]:.1f} m)")

plt.title('M026: 2D Dead-Reckoning Trajectory Comparison', fontsize=12, fontweight='bold')
...
plt.axis('equal')
plt.tight_layout()
plt.savefig('plots/vw4/m026_speed_dependent_nhc/trajectory_comparison.png', dpi=300)
plt.close()

# Save JSON Summary & NPZ Predictions
summary_data = {
    'milestone': 'M026',
    'title': 'Conservative Speed-Dependent NHC Covariance Ablation',
    'm019_benchmark': 220.12,
    'selected_val_gamma': best_val_gamma,
    'results_table': results_table
}

with open('results/vw4_m026_speed_dependent_nhc_summary.json', 'w') as f:
    json.dump(summary_data, f, indent=2)

np.savez(
    'results/vw4_m026_speed_dependent_nhc_predictions.npz',
    test_indices=test_indices,
    v_gt=vbox_vel_ms[test_indices],
    pos_err_f0=nav_err_series['F0 (Control gamma=0.0)'],
    pos_err_f1=nav_err_series['F1 (gamma=0.005)'],
    pos_err_f4=nav_err_series['F4 (gamma=0.050)']
)

print("\nSaved M026 summary JSON and predictions NPZ.")
print("M026 execution complete.")
