...
# ── Navigation Engine for M022 APM Integration Window Ablation ────────────
def run_navigation_m022(
    v_ml_dict, prob_stat_dict, sim_start_idx=108000, duration_sec=300,
    apm_window_samples=5, apm_accel_thresh=-0.5, apm_bound_ms=0.5, apm_w_thresh_deg=3.0
):
    ...
    for idx in range(sim_start, sim_end):
        ...
        if not is_outage:
            ...
            # M022 APM Integration Window Logic
            N_w = apm_window_samples
            if (not is_stat_pred) and (len(v_est_history) >= N_w):
                is_decel = (a_long[idx] < apm_accel_thresh)
                is_turn_ok = (np.abs(w_yaw[idx]) <= np.radians(apm_w_thresh_deg))

                if is_decel and is_turn_ok:
                    apm_stats['activations'] += 1
                    # Causal integration over N_w samples: sum(a_long[idx-N_w+1 : idx+1]) * dt
                    delta_v_imu = np.sum(a_long[idx-N_w+1 : idx+1]) * dt
                    v_anchor = v_est_history[-N_w]
...
def compute_metrics_m022(x_dr, y_dr, v_dr, psi_deg, v_meas_arr, start, dur):
    n = int(dur / dt)
...
xd, yd, vd, pd, _, vm = run_navigation_m022(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=300, apm_window_samples=5)
m_f0 = compute_metrics_m022(xd, yd, vd, pd, vm, start_idx, 300)

p60  = compute_metrics_m022(*run_navigation_m022(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=60, apm_window_samples=5)[:4], run_navigation_m022(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=60, apm_window_samples=5)[5], start_idx, 60)['final_pos_m']
p120 = compute_metrics_m022(*run_navigation_m022(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=120, apm_window_samples=5)[:4], run_navigation_m022(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=120, apm_window_samples=5)[5], start_idx, 120)['final_pos_m']
...
# ── 2. Validation Integration Window Grid Search (Partition 88566:107535) ──
print("======================================================================")
print("2. VALIDATION APM WINDOW EVALUATION (Partition 88566:107535)")
print("======================================================================")
...
val_window_specs = [
    ...
    ('F2 (0.7s / 7 samples)', 7),
    ('F3 (0.8s / 8 samples)', 8),
    ('F4 (1.0s / 10 samples)', 10)
]

for wname, n_w in val_window_specs:
    xd, yd, vd, pd, _, vm = run_navigation_m022(
        v_f4_dict, prob_stat_dict, sim_start_idx=idx_train_end + 300, duration_sec=300,
        apm_window_samples=n_w
    )
    m_val = compute_metrics_m022(xd, yd, vd, pd, vm, idx_train_end + 300, 300)
    print(f"  Val Window: {wname:<24} -> Val 300s Error = {m_val['final_pos_m']:.2f} m", flush=True)
    if m_val['final_pos_m'] < best_val_err:
        best_val_err = m_val['final_pos_m']
        best_val_window = n_w

print(f"  Selected Validation Best Window: N = {best_val_window} samples ({best_val_window*0.1:.1f}s) (Val Error = {best_val_err:.2f} m)\n", flush=True)

# ── 3. Candidate Sweep Execution on Unseen Test set (`start_idx = 108,000`) ──
candidate_specs = [
    {
        'id': 'F0 (Control 0.5s / 5samp)',
        'desc': 'M019 Control (0.5s / 5 causal samples)',
        'kwargs': {'apm_window_samples': 5}
    },
    {
        'id': 'F1 (0.6s / 6 samples)',
        'desc': '0.6s Integration Window (6 causal samples)',
        'kwargs': {'apm_window_samples': 6}
    },
    {
        'id': 'F2 (0.7s / 7 samples)',
        'desc': '0.7s Integration Window (7 causal samples)',
        'kwargs': {'apm_window_samples': 7}
    },
    {
        'id': 'F3 (0.8s / 8 samples)',
        'desc': '0.8s Integration Window (8 causal samples)',
        'kwargs': {'apm_window_samples': 8}
    },
    {
        'id': 'F4 (1.0s / 10 samples)',
        'desc': '1.0s Integration Window Stress Test (10 causal samples)',
        'kwargs': {'apm_window_samples': 10}
    },
    {
        'id': 'F5 (Best Validated + M019)',
        'desc': f"Best Validated Window (N={best_val_window})",
        'kwargs': {'apm_window_samples': best_val_window}
    }
]
...
for spec in candidate_specs:
    ...
    for dur in [60, 120, 300]:
        xd, yd, vd, pd, stats, vm = run_navigation_m022(
            v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=dur,
            **spec['kwargs']
        )
        m = compute_metrics_m022(xd, yd, vd, pd, vm, start_idx, dur)
...
# ── 4. APM Window Diagnostic Metrics Summary ──────────────────────────────
print("\n======================================================================")
print("4. APM WINDOW ABLATION DIAGNOSTIC METRICS SUMMARY")
print("======================================================================")
...
os.makedirs('plots/vw4/m022_apm_window_ablation', exist_ok=True)
...
plt.title('M022: APM Window Length Ablation 300s Position Error', fontsize=12, fontweight='bold')
plt.xlabel('Outage Elapsed Time (s)')
plt.ylabel('Position Error (m)')
plt.grid(True, alpha=0.3)
plt.legend(loc='upper left', fontsize=8)
plt.tight_layout()
plt.savefig('plots/vw4/m022_apm_window_ablation/pos_error_vs_time.png', dpi=300)
plt.close()
...
for cid, (xd, yd) in traj_series.items():
    if cid in ['F0 (Control 0.5s / 5samp)', 'F1 (0.6s / 6 samples)', 'F3 (0.8s / 8 samples)']:
        plt.plot(xd, yd, label=f"{cid} ({nav_err_series[cid][-1]:.1f} m)")

plt.title('M022: 2D Dead-Reckoning Trajectory Comparison', fontsize=12, fontweight='bold')
...
plt.axis('equal')
plt.tight_layout()
plt.savefig('plots/vw4/m022_apm_window_ablation/trajectory_comparison.png', dpi=300)
plt.close()

# Save JSON Summary & NPZ Predictions
summary_data = {
    'milestone': 'M022',
    'title': 'Causal Deceleration-Integrated Speed Damping Window Ablation',
    'm019_benchmark': 220.12,
    'selected_val_window_samples': best_val_window,
    'results_table': results_table
}

with open('results/vw4_m022_apm_window_ablation_summary.json', 'w') as f:
    json.dump(summary_data, f, indent=2)

np.savez(
    'results/vw4_m022_apm_window_ablation_predictions.npz',
    test_indices=test_indices,
    v_gt=vbox_vel_ms[test_indices],
    pos_err_f0=nav_err_series['F0 (Control 0.5s / 5samp)'],
    pos_err_f1=nav_err_series['F1 (0.6s / 6 samples)'],
    pos_err_f3=nav_err_series['F3 (0.8s / 8 samples)']
)

print("\nSaved M022 summary JSON and predictions NPZ.")
print("M022 execution complete.")
