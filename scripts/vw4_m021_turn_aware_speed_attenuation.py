...
# ── Navigation Engine for M021 Turn Attenuation ───────────────────────────
def run_navigation_m021(
    v_ml_dict, prob_stat_dict, sim_start_idx=108000, duration_sec=300,
    atten_mode='none', atten_factor=0.95
):
    ...
    for idx in range(sim_start, sim_end):
        ...
        if not is_outage:
            ...
            # M019 APM Logic (Always active in baseline stack)
            if (not is_stat_pred) and (len(v_est_history) >= 5):
                ...
                if is_decel and is_turn_ok:
                    ...
                    if v_speednet > z_apm:
                        v_speednet = v_speednet - min(v_speednet - z_apm, 0.5)

            # M021 Turn-Aware Attenuation Logic
            v_meas = v_speednet
            w_abs_deg = np.degrees(np.abs(w_yaw[idx]))

            if (not is_stat_pred):
                if atten_mode == 'fixed' and w_abs_deg > 10.0:
                    turn_stats['activations'] += 1
                    v_atten = v_speednet * atten_factor
                    turn_stats['corrections_kmh'].append((v_speednet - v_atten) * 3.6)
                    v_meas = v_atten
                elif atten_mode == 'linear' and w_abs_deg > 10.0:
                    turn_stats['activations'] += 1
                    scale = np.clip((w_abs_deg - 10.0) / 10.0, 0.0, 1.0)
                    atten_frac = 1.0 - 0.15 * scale # max 15% attenuation at 20 deg/s
                    v_atten = v_speednet * atten_frac
                    turn_stats['corrections_kmh'].append((v_speednet - v_atten) * 3.6)
                    v_meas = v_atten
    ...
    return x_dr, y_dr, v_dr, psi_deg, turn_stats, np.array(v_meas_history)

def compute_metrics_m021(x_dr, y_dr, v_dr, psi_deg, v_meas_arr, start, dur):
    n = int(dur / dt)
...
xd, yd, vd, pd, _, vm = run_navigation_m021(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=300, atten_mode='none')
m_f0 = compute_metrics_m021(xd, yd, vd, pd, vm, start_idx, 300)

p60  = compute_metrics_m021(*run_navigation_m021(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=60, atten_mode='none')[:4], run_navigation_m021(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=60, atten_mode='none')[5], start_idx, 60)['final_pos_m']
p120 = compute_metrics_m021(*run_navigation_m021(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=120, atten_mode='none')[:4], run_navigation_m021(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=120, atten_mode='none')[5], start_idx, 120)['final_pos_m']
...
print("======================================================================")
print("2. VALIDATION CANDIDATE EVALUATION (Partition 88566:107535)")
print("======================================================================")
...
val_candidates = [
    ('F0 (Control M019)', 'none', 1.0),
    ('F1 (5% Attenuation)', 'fixed', 0.95),
    ('F2 (10% Attenuation)', 'fixed', 0.90),
    ('F3 (15% Attenuation)', 'fixed', 0.85),
    ('F4 (Linear 10-20 deg/s)', 'linear', 0.85)
]

for cname, amode, afactor in val_candidates:
    xd, yd, vd, pd, _, vm = run_navigation_m021(
        v_f4_dict, prob_stat_dict, sim_start_idx=idx_train_end + 300, duration_sec=300,
        atten_mode=amode, atten_factor=afactor
    )
    m_val = compute_metrics_m021(xd, yd, vd, pd, vm, idx_train_end + 300, 300)
    print(f"  Val Candidate: {cname:<24} -> Val 300s Error = {m_val['final_pos_m']:.2f} m", flush=True)
    if m_val['final_pos_m'] < best_val_err:
        best_val_err = m_val['final_pos_m']
        best_val_cand = (cname, amode, afactor)

print(f"  Selected Validation Best Candidate: {best_val_cand[0]} (Val Error = {best_val_err:.2f} m)\n", flush=True)

# ── 3. Candidate Sweep Execution on Unseen Test set (`start_idx = 108,000`) ──
candidate_specs = [
    {
        'id': 'F0 (Control M019)',
        'desc': 'SpeedNet v2 W=40 Baseline Control (No Turn Attenuation)',
        'kwargs': {'atten_mode': 'none', 'atten_factor': 1.0}
    },
    {
        'id': 'F1 (5% Turn Atten)',
        'desc': '5% Attenuation when |w_y| > 10 deg/s',
        'kwargs': {'atten_mode': 'fixed', 'atten_factor': 0.95}
    },
    {
        'id': 'F2 (10% Turn Atten)',
        'desc': '10% Attenuation when |w_y| > 10 deg/s',
        'kwargs': {'atten_mode': 'fixed', 'atten_factor': 0.90}
    },
    {
        'id': 'F3 (15% Turn Atten)',
        'desc': '15% Attenuation when |w_y| > 10 deg/s',
        'kwargs': {'atten_mode': 'fixed', 'atten_factor': 0.85}
    },
    {
        'id': 'F4 (Linear 10-20 deg/s)',
        'desc': 'Bounded Linear Attenuation (0-15% for 10-20 deg/s)',
        'kwargs': {'atten_mode': 'linear', 'atten_factor': 0.85}
    },
    {
        'id': 'F5 (Best Validated + M019)',
        'desc': f"Best Validated Candidate ({best_val_cand[0]})",
        'kwargs': {'atten_mode': best_val_cand[1], 'atten_factor': best_val_cand[2]}
    }
]
...
print("======================================================================")
print(f"{'Variant Name':<26} | {'Speed MAE':<9} | {'St-Turn MAE':<11} | {'60s (m)':<7} | {'120s (m)':<8} | {'300s (m)':<8} | {'vs 263.11m'} | {'vs 254.11m'} | {'vs 220.12m'}")
print("-" * 125)
...
for spec in candidate_specs:
    ...
    for dur in [60, 120, 300]:
        xd, yd, vd, pd, stats, vm = run_navigation_m021(
            v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=dur,
            **spec['kwargs']
        )
        m = compute_metrics_m021(xd, yd, vd, pd, vm, start_idx, dur)
    ...
    turn_diagnostics_dict[cid] = stats_300

    v_diff_sturn = (vm_300[mask_sturn] - v_gt_test[mask_sturn]) * 3.6
    sturn_mae  = float(np.mean(np.abs(v_diff_sturn)))
    sturn_bias = float(np.mean(v_diff_sturn))
    ...
    results_table.append({
        ...
        'sturn_mae_kmh': round(sturn_mae, 2),
        'sturn_bias_kmh': round(sturn_bias, 2),
        'err_60s': dur_res[60],
    ...
    print(f"{cid:<26} | {m_300['speed_mae_kmh']:>6.2f}k  | {sturn_mae:>8.2f}k    | {dur_res[60]:>7.2f} | {dur_res[120]:>8.2f} | {dur_res[300]:>8.2f} | {pct_263:>+9.1f}% | {pct_254:>+9.1f}% | {b_220:>10}")

# ── 4. Turn Attenuation Diagnostic Metrics ─────────────────────────────────
print("\n======================================================================")
print("4. TURN ATTENUATION DIAGNOSTIC METRICS SUMMARY")
print("======================================================================")
print(f"{'Candidate':<26} | {'Activations':<12} | {'Mean Corr (km/h)':<18} | {'Max Corr (km/h)'}")
print("-" * 75)
for cid, stats in turn_diagnostics_dict.items():
    ...
    print(f"{cid:<26} | {act:>12d} | {m_corr:>18.2f} | {mx_corr:>15.2f}")

# ── Save Diagnostic Plots ─────────────────────────────────────────────────
os.makedirs('plots/vw4/m021_turn_aware_speed_attenuation', exist_ok=True)
...
plt.title('M021: Turn-Aware Speed Attenuation 300s Position Error', fontsize=12, fontweight='bold')
plt.xlabel('Outage Elapsed Time (s)')
plt.ylabel('Position Error (m)')
plt.grid(True, alpha=0.3)
plt.legend(loc='upper left', fontsize=8)
plt.tight_layout()
plt.savefig('plots/vw4/m021_turn_aware_speed_attenuation/pos_error_vs_time.png', dpi=300)
plt.close()
...
for cid, (xd, yd) in traj_series.items():
    if cid in ['F0 (Control M019)', 'F1 (5% Turn Atten)', 'F4 (Linear 10-20 deg/s)']:
...
plt.axis('equal')
plt.tight_layout()
plt.savefig('plots/vw4/m021_turn_aware_speed_attenuation/trajectory_comparison.png', dpi=300)
plt.close()

# Save JSON Summary & NPZ Predictions
summary_data = {
    'milestone': 'M021',
    'title': 'Conservative Turn-Aware Speed Attenuation Ablation',
    'm019_benchmark': 220.12,
    'selected_val_candidate': best_val_cand[0],
    'results_table': results_table
}

with open('results/vw4_m021_turn_aware_speed_attenuation_summary.json', 'w') as f:
    json.dump(summary_data, f, indent=2)

np.savez(
    'results/vw4_m021_turn_aware_speed_attenuation_predictions.npz',
    test_indices=test_indices,
    v_gt=vbox_vel_ms[test_indices],
    pos_err_f0=nav_err_series['F0 (Control M019)'],
    pos_err_f1=nav_err_series['F1 (5% Turn Atten)'],
    pos_err_f4=nav_err_series['F4 (Linear 10-20 deg/s)']
)

print("\nSaved M021 summary JSON and predictions NPZ.")
print("M021 execution complete.")
