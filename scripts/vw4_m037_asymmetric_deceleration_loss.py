...
# ── Training Function for Asymmetric Loss ──────────────────────────────────
def train_asymmetric_speednet(lambda_asym=0.0, window_size=40, num_epochs=15, batch_size=1024, lr=1e-3):
    model_path = f'models/m037_speednet_w40_lambda_{str(lambda_asym).replace(".", "")}.pth'
    if os.path.exists(model_path) and lambda_asym > 0:
        print(f"Loading existing trained model for lambda={lambda_asym} from {model_path}...")
        return model_path

    print(f"\n--- Training SpeedNet v2 (W={window_size}, lambda_asym={lambda_asym}) ---", flush=True)
    ...
    v_tr_tensor     = torch.tensor(vbox_vel_ms[train_indices], dtype=torch.float32)
    ...
    v_val_tensor     = torch.tensor(vbox_vel_ms[val_indices], dtype=torch.float32)
    ...
    train_dataset = TensorDataset(X_tr_tensor, v_tr_tensor, w_tr_tensor, stat_tr_tensor, delta_tr_tensor, along_tr_tensor)
    val_dataset   = TensorDataset(X_val_tensor, v_val_tensor, w_val_tensor, stat_val_tensor, delta_val_tensor, along_val_tensor)
    ...
    for epoch in range(1, num_epochs + 1):
        model.train()
        train_loss = 0.0
        for x_b, v_b, w_b, stat_b, delta_b, along_b in train_loader:
            x_b, v_b, w_b, stat_b, delta_b, along_b = (
                x_b.to(device), v_b.to(device), w_b.to(device),
                stat_b.to(device), delta_b.to(device), along_b.to(device)
            )
            optimizer.zero_grad()
            ...
            # Asymmetric Deceleration Loss Penalty
            mask_decel = (along_b < -0.5).float()
            over_pred  = torch.relu(v_pred - v_b)
            l_asym = lambda_asym * torch.mean(torch.square(over_pred) * mask_decel)

            loss = 1.0 * l_v + 0.5 * l_w + 0.5 * l_stat + 0.2 * l_delta + l_asym
            loss.backward()
            optimizer.step()
        ...
        val_speed_err = 0.0
        with torch.no_grad():
            for x_b, v_b, w_b, stat_b, delta_b, along_b in val_loader:
                x_b, v_b, w_b, stat_b, delta_b, along_b = (
                    x_b.to(device), v_b.to(device), w_b.to(device),
                    stat_b.to(device), delta_b.to(device), along_b.to(device)
                )
...
# Train candidates F1 - F4
print("Setting up M037 candidate models...", flush=True)
candidate_configs = [
    {'id': 'F0 (Control Baseline)', 'lambda': 0.000, 'path': 'models/speednet_v2_w40.pth'},
    {'id': 'F1 (lambda=0.001)',     'lambda': 0.001, 'path': train_asymmetric_speednet(0.001)},
    {'id': 'F2 (lambda=0.003)',     'lambda': 0.003, 'path': train_asymmetric_speednet(0.003)},
    {'id': 'F3 (lambda=0.010)',     'lambda': 0.010, 'path': train_asymmetric_speednet(0.010)},
    {'id': 'F4 (lambda=0.030)',     'lambda': 0.030, 'path': train_asymmetric_speednet(0.030)}
]

candidate_models = []
for c in candidate_configs:
    v_raw, w_raw, prob_stat = get_speednet_predictions(40, c['path'])
    v_f4 = get_f4_speed_dict(v_raw)
...
def compute_metrics_m037(x_dr, y_dr, v_dr, psi_deg, v_meas_arr, start, dur):
    n = int(dur / dt)
...
xd, yd, vd, pd_deg, vm = run_navigation_m037(candidate_models[0]['v_dict'], candidate_models[0]['p_dict'], sim_start_idx=start_idx, duration_sec=300)
m_f0 = compute_metrics_m037(xd, yd, vd, pd_deg, vm, start_idx, 300)

p60  = compute_metrics_m037(*run_navigation_m037(candidate_models[0]['v_dict'], candidate_models[0]['p_dict'], sim_start_idx=start_idx, duration_sec=60)[:4], run_navigation_m037(candidate_models[0]['v_dict'], candidate_models[0]['p_dict'], sim_start_idx=start_idx, duration_sec=60)[4], start_idx, 60)['final_pos_m']
p120 = compute_metrics_m037(*run_navigation_m037(candidate_models[0]['v_dict'], candidate_models[0]['p_dict'], sim_start_idx=start_idx, duration_sec=120)[:4], run_navigation_m037(candidate_models[0]['v_dict'], candidate_models[0]['p_dict'], sim_start_idx=start_idx, duration_sec=120)[4], start_idx, 120)['final_pos_m']
...
# ── PHASE 4: POINTWISE SPEED & REGIME MAE ANALYSIS (Validation Set) ───────
print("======================================================================")
print("PHASE 4: POINTWISE SPEED & REGIME MAE ANALYSIS (Validation Set 88566:107535)")
print("======================================================================")
...
print(f"{'Variant Name':<32} | {'Overall MAE':<11} | {'Braking MAE':<11} | {'Braking Bias':<12} | {'Accel MAE':<11} | {'Turn MAE':<11}")
print("-" * 110)
...
for spec in candidate_models:
    ...
    b_val = (v_pred_val - v_gt_val) * 3.6

    ov_mae   = float(np.mean(e_val))
    brk_mae  = float(np.mean(e_val[mask_brk_val]))
    brk_bias = float(np.mean(b_val[mask_brk_val]))
    acc_mae  = float(np.mean(e_val[mask_acc_val]))
    trn_mae  = float(np.mean(e_val[mask_trn_val]))

    print(f"{spec['name']:<32} | {ov_mae:>7.2f} km/h | {brk_mae:>7.2f} km/h | {brk_bias:>+8.2f} km/h | {acc_mae:>7.2f} km/h | {trn_mae:>7.2f} km/h")
...
best_val_name = candidate_models[0]['name']
best_spec = candidate_models[0]

val_nav_results = []
for spec in candidate_models:
    xd, yd, vd, pd_deg, vm = run_navigation_m037(
        spec['v_dict'], spec['p_dict'], sim_start_idx=idx_train_end + 300, duration_sec=300
    )
    m_val = compute_metrics_m037(xd, yd, vd, pd_deg, vm, idx_train_end + 300, 300)
    val_nav_results.append((spec['id'], spec['name'], m_val['final_pos_m'], m_val['mean_pos_m']))
    print(f"  Val Candidate: {spec['name']:<35} -> Val 300s Error = {m_val['final_pos_m']:.2f} m (Mean = {m_val['mean_pos_m']:.2f} m)", flush=True)
...
for spec in candidate_models:
    ...
    for dur in [60, 120, 300]:
        ...
        m = compute_metrics_m037(xd, yd, vd, pd_deg, vm, start_idx, dur)
...
os.makedirs('plots/vw4/m037_asymmetric_deceleration_loss', exist_ok=True)
...
plt.title('M037: Asymmetric Deceleration Loss Ablation 300s Position Error', fontsize=12, fontweight='bold')
plt.xlabel('Outage Elapsed Time (s)')
plt.ylabel('Position Error (m)')
plt.grid(True, alpha=0.3)
plt.legend(loc='upper left', fontsize=8)
plt.tight_layout()
plt.savefig('plots/vw4/m037_asymmetric_deceleration_loss/pos_error_vs_time.png', dpi=300)
plt.close()

# Save Summary JSON
summary_data = {
    'milestone': 'M037',
    'title': 'Asymmetric Deceleration Loss Ablation',
...
with open('results/vw4_m037_asymmetric_deceleration_loss_summary.json', 'w') as f:
    json.dump(summary_data, f, indent=2)

print("\nSaved M037 summary JSON and diagnostic plot.")
print("M037 execution complete.")
