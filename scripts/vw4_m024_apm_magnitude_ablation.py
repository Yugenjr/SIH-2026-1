...
# ── Navigation Engine for M024 APM Magnitude Ablation ─────────────────────
def run_navigation_m024(
...
def compute_metrics_m024(x_dr, y_dr, v_dr, psi_deg, v_meas_arr, start, dur):
    n = int(dur / dt)
...
xd, yd, vd, pd, _, vm = run_navigation_m024(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=300, apm_bound_ms=0.50)
m_f0 = compute_metrics_m024(xd, yd, vd, pd, vm, start_idx, 300)

p60  = compute_metrics_m024(*run_navigation_m024(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=60, apm_bound_ms=0.50)[:4], run_navigation_m024(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=60, apm_bound_ms=0.50)[5], start_idx, 60)['final_pos_m']
p120 = compute_metrics_m024(*run_navigation_m024(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=120, apm_bound_ms=0.50)[:4], run_navigation_m024(v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=120, apm_bound_ms=0.50)[5], start_idx, 120)['final_pos_m']
...
# ── 2. Validation Search for Best APM Bound (Partition 88566:107535) ───────
print("======================================================================")
print("2. VALIDATION APM MAGNITUDE EVALUATION (Partition 88566:107535)")
print("======================================================================")
...
val_magnitude_specs = [
    ...
    ('F2 (0.35 m/s)', 0.35),
    ('F3 (0.40 m/s)', 0.40),
    ('F0 (Control 0.50 m/s)', 0.50),
    ('F4 (0.60 m/s)', 0.60),
    ('F5 (0.75 m/s)', 0.75)
]

for bname, bval in val_magnitude_specs:
    xd, yd, vd, pd, _, vm = run_navigation_m024(
        v_f4_dict, prob_stat_dict, sim_start_idx=idx_train_end + 300, duration_sec=300,
        apm_bound_ms=bval
    )
    m_val = compute_metrics_m024(xd, yd, vd, pd, vm, idx_train_end + 300, 300)
    print(f"  Val Bound: {bname:<24} -> Val 300s Error = {m_val['final_pos_m']:.2f} m", flush=True)
    if m_val['final_pos_m'] < best_val_err:
        best_val_err = m_val['final_pos_m']
        best_val_bound = bval

print(f"  Selected Validation Best Bound: delta_v_max = {best_val_bound:.2f} m/s ({best_val_bound*3.6:.2f} km/h) (Val Error = {best_val_err:.2f} m)\n", flush=True)

# ── 3. Candidate Sweep Execution on Unseen Test set (`start_idx = 108,000`) ──
candidate_specs = [
    {
        'id': 'F0 (Control 0.50 m/s)',
        'desc': 'M019 Baseline Control (delta_v_max = 0.50 m/s)',
        'kwargs': {'apm_bound_ms': 0.50}
    },
    {
        'id': 'F1 (0.25 m/s)',
        'desc': 'delta_v_max = 0.25 m/s (0.90 km/h)',
        'kwargs': {'apm_bound_ms': 0.25}
    },
    {
        'id': 'F2 (0.35 m/s)',
        'desc': 'delta_v_max = 0.35 m/s (1.26 km/h)',
        'kwargs': {'apm_bound_ms': 0.35}
    },
    {
        'id': 'F3 (0.40 m/s)',
        'desc': 'delta_v_max = 0.40 m/s (1.44 km/h)',
        'kwargs': {'apm_bound_ms': 0.40}
    },
    {
        'id': 'F4 (0.60 m/s)',
        'desc': 'delta_v_max = 0.60 m/s (2.16 km/h)',
        'kwargs': {'apm_bound_ms': 0.60}
    },
    {
        'id': 'F5 (0.75 m/s)',
        'desc': 'delta_v_max = 0.75 m/s (2.70 km/h)',
        'kwargs': {'apm_bound_ms': 0.75}
    },
    {
        'id': 'F6 (Best Validated + M019)',
        'desc': f"Best Validated Bound (delta_v_max = {best_val_bound:.2f} m/s)",
        'kwargs': {'apm_bound_ms': best_val_bound}
    }
]
...
for spec in candidate_specs:
    ...
    for dur in [60, 120, 300]:
        xd, yd, vd, pd, stats, vm = run_navigation_m024(
            v_f4_dict, prob_stat_dict, sim_start_idx=start_idx, duration_sec=dur,
            **spec['kwargs']
        )
        m = compute_metrics_m024(xd, yd, vd, pd, vm, start_idx, dur)
...
# ── 4. APM Magnitude Diagnostic Summary ────────────────────────────────────
print("\n======================================================================")
print("4. APM MAGNITUDE ABLATION DIAGNOSTIC METRICS SUMMARY")
print("======================================================================")
...
os.makedirs('plots/vw4/m024_apm_magnitude_ablation', exist_ok=True)
...
plt.title('M024: APM Correction Magnitude Ablation 300s Position Error', fontsize=12, fontweight='bold')
plt.xlabel('Outage Elapsed Time (s)')
plt.ylabel('Position Error (m)')
plt.grid(True, alpha=0.3)
plt.legend(loc='upper left', fontsize=8)
plt.tight_layout()
plt.savefig('plots/vw4/m024_apm_magnitude_ablation/pos_error_vs_time.png', dpi=300)
plt.close()
...
for cid, (xd, yd) in traj_series.items():
    if cid in ['F0 (Control 0.50 m/s)', 'F1 (0.25 m/s)', 'F5 (0.75 m/s)']:
        plt.plot(xd, yd, label=f"{cid} ({nav_err_series[cid][-1]:.1f} m)")

plt.title('M024: 2D Dead-Reckoning Trajectory Comparison', fontsize=12, fontweight='bold')
...
plt.axis('equal')
plt.tight_layout()
plt.savefig('plots/vw4/m024_apm_magnitude_ablation/trajectory_comparison.png', dpi=300)
plt.close()

# Save JSON Summary & NPZ Predictions
summary_data = {
    'milestone': 'M024',
    'title': 'APM Correction-Magnitude Ablation',
    'm019_benchmark': 220.12,
    'selected_val_bound': best_val_bound,
    'results_table': results_table
}

with open('results/vw4_m024_apm_magnitude_ablation_summary.json', 'w') as f:
    json.dump(summary_data, f, indent=2)

np.savez(
    'results/vw4_m024_apm_magnitude_ablation_predictions.npz',
    test_indices=test_indices,
    v_gt=vbox_vel_ms[test_indices],
    pos_err_f0=nav_err_series['F0 (Control 0.50 m/s)'],
    pos_err_f1=nav_err_series['F1 (0.25 m/s)'],
    pos_err_f5=nav_err_series['F5 (0.75 m/s)']
)

print("\nSaved M024 summary JSON and predictions NPZ.")
print("M024 execution complete.")
