...
os.makedirs('plots/vw4/ml_nhc_heading_filter', exist_ok=True)
...
# Pre-outage gyro bias estimation
b_gyro_static = float(np.mean(w_yaw[start_idx - 100 : start_idx]))

# --- Case A: ML Speed + ML Yaw (Baseline Open-Loop DR) ---
def run_case_a(start_idx, duration_sec):
    ...
    for k in range(1, n):
        ...
        # Residual of lateral velocity (0 in 2D DR model)
        v_lat_res[k] = 0.0
    t1 = time.perf_counter()
    return x_dr, y_dr, v_dr, np.degrees(psi_dr), w_dr, v_lat_res, ((t1 - t0) / n) * 1000.0

# --- EKF Engine for Cases B, C, D ---
def run_ekf_case(start_idx, duration_sec, case_type='B'):
    n = int(duration_sec / dt)
    pre_samples = 300
    ...
    for idx in range(sim_start, sim_end):
        ...
        if not is_outage:
            ...
            # Update 2: Kinematic NHC Lateral Velocity Constraint (Case B, C, D)
            psi_c = x_state[4]
            ...
            # Update 3: Straight-Road Heading Anchor (Case C and Case D)
            if case_type in ['C', 'D']:
                if abs(w_ml) < np.radians(0.5) and v_ml > 1.0:
                    # Straight road anchor: zero yaw rate -> update b_w
            ...
            # Update 4: Tilt-Compensated Centrifugal Cross-Check (Case D)
            if case_type == 'D':
                a_lat_tc = a_lat_tilt_comp[idx]
                if v_ml > 2.0 and abs(a_lat_tc) > 0.1:
                    w_centrifugal = a_lat_tc / v_ml
                    y_w = (w_m - w_centrifugal) - x_state[6]
                    H_w = np.array([0, 0, 0, 0, 0, 0, 1.0])
                    R_w = np.radians(1.0)**2
                    S_w = float(H_w @ P @ H_w.T + R_w)
                    K_w = (P @ H_w.T) / S_w
                    x_state = x_state + K_w * y_w
                    P = (np.eye(7) - np.outer(K_w, H_w)) @ P
                    
            v_lat_res_hist.append(abs(v_lat_est))
    ...
    return x_ekf, y_ekf, v_ekf, psi_ekf_deg, np.array(w_hist), np.array(v_lat_res_hist), lat_ms

# --- Case E: ML Speed + TRUE Heading (Oracle Error Decomposition Reference ONLY) ---
def run_case_e_oracle(start_idx, duration_sec):
    n = int(duration_sec / dt)
    v_dr = np.array([v_ml_dict[start_idx + k] for k in range(n)])
    ...
    return x_dr, y_dr, v_dr, psi_gt_deg, w_gt, v_lat_res, ((t1 - t0) / n) * 1000.0

# --- Metrics Evaluator ---
def evaluate_dr(x_dr, y_dr, v_dr, psi_dr_deg, w_dr_rads, v_lat_res, start_idx, duration_sec, case_name):
    n = int(duration_sec / dt)
    ...
    w_err_arr = np.degrees(np.abs(w_dr_rads - w_gt_rads))
    w_mae_degs = float(np.mean(w_err_arr))
    w_rmse_degs = float(np.sqrt(np.mean(w_err_arr**2)))
    
    nhc_res_mae_ms = float(np.mean(v_lat_res))
    
    # Filter numerical stability check
    is_stable = not (np.any(np.isnan(pos_err)) or np.any(np.isinf(pos_err)) or max_pos_err > 50000.0)
    stability_status = "Stable" if is_stable else "Diverged"
    ...
    return {
        ...
        'nhc_res_mae_ms': round(nhc_res_mae_ms, 4),
        'filter_status': stability_status,
        'x_dr': x_dr,
        'y_dr': y_dr,
        'v_dr': v_dr,
        'w_dr': w_dr_rads,
...
cases = [
    ("Case A: ML Speed + ML Yaw (Baseline)", 'A'),
    ("Case B: ML Speed + NHC", 'B'),
    ("Case C: ML Speed + NHC + Heading Anchor", 'C'),
    ("Case D: ML Speed + NHC + Adaptive Heading", 'D'),
    ("Case E: ML Speed + TRUE Heading (Oracle Ref)", 'E')
]
...
for dur in [60, 120, 300]:
    ...
    # Case A
    xa, ya, va, psia, wa, res_a, lat_a = run_case_a(start_idx, dur)
    r_a = evaluate_dr(xa, ya, va, psia, wa, res_a, start_idx, dur, cases[0][0])
    r_a['latency_ms'] = round(lat_a, 4)
    
    # Case B
    xb, yb, vb, psib, wb, res_b, lat_b = run_ekf_case(start_idx, dur, 'B')
    r_b = evaluate_dr(xb, yb, vb, psib, wb, res_b, start_idx, dur, cases[1][0])
    r_b['latency_ms'] = round(lat_b, 4)
    
    # Case C
    xc, yc, vc, psic, wc, res_c, lat_c = run_ekf_case(start_idx, dur, 'C')
    r_c = evaluate_dr(xc, yc, vc, psic, wc, res_c, start_idx, dur, cases[2][0])
    r_c['latency_ms'] = round(lat_c, 4)
    
    # Case D
    xd, yd, vd, psid, wd, res_d, lat_d = run_ekf_case(start_idx, dur, 'D')
    r_d = evaluate_dr(xd, yd, vd, psid, wd, res_d, start_idx, dur, cases[3][0])
    r_d['latency_ms'] = round(lat_d, 4)
    
    # Case E (Oracle Reference)
    xe, ye, ve, psie, we, res_e, lat_e = run_case_e_oracle(start_idx, dur)
    r_e = evaluate_dr(xe, ye, ve, psie, we, res_e, start_idx, dur, cases[4][0])
    r_e['latency_ms'] = round(lat_e, 4)
    
    plot_data[dur] = [r_a, r_b, r_c, r_d, r_e]
    all_results.extend([r_a, r_b, r_c, r_d, r_e])
...
with open('results/vw4_ml_nhc_heading_filter_results.json', 'w') as f:
    json.dump(json_results, f, indent=2)

df_res = pd.DataFrame(json_results)[['duration_sec', 'case_name', 'cde_pct', 'final_pos_err_m', 'max_pos_err_m', 'drift_rate_ms', 'v_mae_kmh', 'final_h_err_deg', 'nhc_res_mae_ms', 'filter_status', 'latency_ms']]
print("\n" + "="*145)
print("VW4 UNSEEN TEST SET: EKF-BASED NHC + HEADING ANCHOR FILTER ABLATION MATRIX")
print("="*145)
...
case_colors = {
    "Case A: ML Speed + ML Yaw (Baseline)": "#9b59b6",
    "Case B: ML Speed + NHC": "#3498db",
    "Case C: ML Speed + NHC + Heading Anchor": "#e67e22",
    "Case D: ML Speed + NHC + Adaptive Heading": "#27ae60",
    "Case E: ML Speed + TRUE Heading (Oracle Ref)": "#e74c3c"
}

case_styles = {
    "Case A: ML Speed + ML Yaw (Baseline)": ":",
    "Case B: ML Speed + NHC": "--",
    "Case C: ML Speed + NHC + Heading Anchor": "-.",
    "Case D: ML Speed + NHC + Adaptive Heading": "-",
    "Case E: ML Speed + TRUE Heading (Oracle Ref)": "-"
}
...
for dur in [60, 120, 300]:
    ...
    plt.tight_layout()
    plt.savefig(f'plots/vw4/ml_nhc_heading_filter/outage_{dur}s_trajectory.png', dpi=300)
    plt.close()
...
ax.legend(fontsize=8, loc='best')
plt.tight_layout()
plt.savefig('plots/vw4/ml_nhc_heading_filter/outage_300s_position_error.png', dpi=300)
plt.close()
...
ax.legend(fontsize=8, loc='best')
plt.tight_layout()
plt.savefig('plots/vw4/ml_nhc_heading_filter/outage_300s_heading_error.png', dpi=300)
plt.close()

# Plot 6: 300s Speed Error Growth
fig, ax = plt.subplots(figsize=(10, 6))
for r in runs300:
    c_name = r['case_name']
...
ax.legend(fontsize=8, loc='best')
plt.tight_layout()
plt.savefig('plots/vw4/ml_nhc_heading_filter/outage_300s_speed_error.png', dpi=300)
plt.close()

# Plot 7: 300s NHC Lateral Velocity Residual
fig, ax = plt.subplots(figsize=(10, 6))
for r in runs300[1:4]: # Cases B, C, D
    c_name = r['case_name']
    ax.plot(t_arr300, r['v_lat_res'], color=case_colors[c_name], linestyle=case_styles[c_name], linewidth=1.8, label=f"{c_name} Residual")
ax.set_title('Vw4 Unseen Test Set: 300s Outage NHC Lateral Velocity Residual v_lat (m/s)', fontsize=14, fontweight='bold')
ax.set_xlabel('Outage Time (seconds)')
ax.set_ylabel('Lateral Velocity Residual |v_lat| (m/s)')
ax.grid(True, linestyle='--', alpha=0.5)
ax.legend(fontsize=8, loc='best')
plt.tight_layout()
plt.savefig('plots/vw4/ml_nhc_heading_filter/outage_300s_nhc_lateral_velocity.png', dpi=300)
plt.close()

# Plot 8: Summary Metrics Comparison Bar Chart
fig, axs = plt.subplots(1, 2, figsize=(15, 6))
fig.suptitle('EKF-Based NHC + Heading Filter Ablation Summary', fontsize=14, fontweight='bold')
...
summary_plot_file = 'plots/vw4/ml_nhc_heading_filter/summary_metrics_comparison.png'
