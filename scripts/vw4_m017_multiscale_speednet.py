...
# ── SpeedNet Prediction Function ──────────────────────────────────────────
def get_speednet_predictions(window_size=40, model_path=None):
...
print("Loading SpeedNet v2 predictions for W=20, W=30, W=40...", flush=True)
v_w20_raw, _, prob_w20 = get_speednet_predictions(window_size=20)
v_w30_raw, _, prob_w30 = get_speednet_predictions(window_size=30)
v_w40_raw, _, prob_w40 = get_speednet_predictions(window_size=40)
...
# Helper function to construct M013 F4 Confidence-Gated Speed Dictionary
...
v_w20_f4 = build_f4_constrained_dict(v_w20_raw)
v_w30_f4 = build_f4_constrained_dict(v_w30_raw)
v_w40_f4 = build_f4_constrained_dict(v_w40_raw)

# ── Validation Search for Two-Scale Ensemble (F4 & F5) ────────────────────
val_indices = np.arange(idx_train_end, idx_val_end)
v_gt_val = vbox_vel_ms[val_indices]

print("\n======================================================================")
print("1. VALIDATION ENSEMBLE GRID SEARCH (Partition 88566:107535)")
print("======================================================================")
...
for alpha in [0.25, 0.50, 0.75]:
    val_preds = np.array([alpha * v_w20_f4.get(i, 0.0) + (1.0 - alpha) * v_w40_f4.get(i, 0.0) for i in val_indices])
    mae = float(np.mean(np.abs(val_preds - v_gt_val)) * 3.6)
    print(f"  Ensemble F4 Grid Search: alpha = {alpha:.2f} -> Val Speed MAE = {mae:.2f} km/h", flush=True)
    if mae < best_val_mae:
        best_val_mae = mae
        best_alpha = alpha

print(f"  Selected F4 Alpha via Validation: alpha = {best_alpha:.2f}", flush=True)

# Build F4 Two-Scale Ensemble Dictionary
v_f4_ens_dict = {}
for idx in range(n_total):
    v_f4_ens_dict[idx] = best_alpha * v_w20_f4.get(idx, 0.0) + (1.0 - best_alpha) * v_w40_f4.get(idx, 0.0)

# Build F5 Braking-Selective Ensemble Dictionary
v_f5_ens_dict = {}
for idx in range(n_total):
    # Causal deceleration detection: a_long < -0.3 m/s^2
    is_decel = (a_long[idx] < -0.3)
    alpha_t = 0.75 if is_decel else 0.0
    v_f5_ens_dict[idx] = alpha_t * v_w20_f4.get(idx, 0.0) + (1.0 - alpha_t) * v_w40_f4.get(idx, 0.0)

# ── Navigation & EKF Engine for M017 (Frozen M014 Stack) ──────────────────
def run_navigation_m017(v_ml_dict, prob_stat_dict, sim_start_idx=108000, duration_sec=300):
...
def compute_metrics_m017(x_dr, y_dr, v_dr, psi_deg, v_ml_dict, start, dur):
    n = int(dur / dt)
...
# ── 2. Prove M014 Control Reproduction Exactly ────────────────────────────
print("\n======================================================================")
print("2. M014 CONTROL REPRODUCTION VERIFICATION")
print("======================================================================")

xd, yd, vd, pd, _ = run_navigation_m017(v_w40_f4, prob_w40, sim_start_idx=start_idx, duration_sec=300)
m_f0 = compute_metrics_m017(xd, yd, vd, pd, v_w40_f4, start_idx, 300)

p60  = compute_metrics_m017(*run_navigation_m017(v_w40_f4, prob_w40, sim_start_idx=start_idx, duration_sec=60)[:4], v_w40_f4, start_idx, 60)['final_pos_m']
p120 = compute_metrics_m017(*run_navigation_m017(v_w40_f4, prob_w40, sim_start_idx=start_idx, duration_sec=120)[:4], v_w40_f4, start_idx, 120)['final_pos_m']
...
candidate_specs = [
    {
        ...
        'v_dict': v_w40_f4,
        'prob_dict': prob_w40,
        'desc': 'SpeedNet v2 W=40 Baseline Control'
    },
    {
        'id': 'F1 (SpeedNet W=20)',
        'v_dict': v_w20_f4,
        'prob_dict': prob_w20,
        'desc': 'SpeedNet v2 W=20 (Short Temporal Context)'
    },
    {
        'id': 'F2 (SpeedNet W=30)',
        'v_dict': v_w30_f4,
        'prob_dict': prob_w30,
        'desc': 'SpeedNet v2 W=30 (Mid Temporal Context)'
    },
    {
        'id': 'F3 (SpeedNet W=40 Ref)',
        'v_dict': v_w40_f4,
        'prob_dict': prob_w40,
        'desc': 'SpeedNet v2 W=40 Reference'
    },
    {
        'id': 'F4 (W20+W40 Ensemble)',
        'v_dict': v_f4_ens_dict,
        'prob_dict': prob_w40,
        'desc': f'Two-Scale Ensemble (alpha={best_alpha:.2f})'
    },
    {
        'id': 'F5 (Braking Ensemble)',
        'v_dict': v_f5_ens_dict,
        'prob_dict': prob_w40,
        'desc': 'Braking-Selective Two-Scale Ensemble'
    }
]
...
print("======================================================================")
print(f"{'Variant Name':<28} | {'Speed MAE':<9} | {'Brake MAE':<10} | {'60s (m)':<7} | {'120s (m)':<8} | {'300s (m)':<8} | {'vs 263.11m'} | {'vs 254.11m'} | {'vs 233.18m'}")
print("-" * 125)

braking_analysis_dict = {}
...
for spec in candidate_specs:
    ...
    for dur in [60, 120, 300]:
        xd, yd, vd, pd, _ = run_navigation_m017(
            spec['v_dict'], spec['prob_dict'], sim_start_idx=start_idx, duration_sec=dur
        )
        m = compute_metrics_m017(xd, yd, vd, pd, spec['v_dict'], start_idx, dur)
    ...
    brk_p95  = float(np.percentile(np.abs(v_diff_brk), 95))

    braking_analysis_dict[cid] = {
        'brk_mae_kmh': round(brk_mae, 2),
        'brk_bias_kmh': round(brk_bias, 2),
        'brk_p95_kmh': round(brk_p95, 2)
    }
    ...
    print(f"{cid:<28} | {m_300['speed_mae_kmh']:>6.2f}k  | {brk_mae:>7.2f}k   | {dur_res[60]:>7.2f} | {dur_res[120]:>8.2f} | {dur_res[300]:>8.2f} | {pct_263:>+9.1f}% | {pct_254:>+9.1f}% | {b_233:>10}")

# ── 4. Detailed Braking & Regime Breakdown Table ──────────────────────────
print("\n======================================================================")
print("4. DRIVING REGIME SPEED MAE BREAKDOWN (km/h)")
print("======================================================================")
print(f"{'Regime':<18} | {'F0 (W=40)':<10} | {'F1 (W=20)':<10} | {'F2 (W=30)':<10} | {'F4 (Ensemble)':<12} | {'F5 (Braking Ens)'}")
print("-" * 90)
for r_name, r_mask in regimes.items():
    if np.sum(r_mask) > 0:
        mae_f0 = float(np.mean(np.abs(np.array([v_w40_f4[i] for i in test_indices])[r_mask] - v_gt_test[r_mask])) * 3.6)
        mae_f1 = float(np.mean(np.abs(np.array([v_w20_f4[i] for i in test_indices])[r_mask] - v_gt_test[r_mask])) * 3.6)
        mae_f2 = float(np.mean(np.abs(np.array([v_w30_f4[i] for i in test_indices])[r_mask] - v_gt_test[r_mask])) * 3.6)
        mae_f4 = float(np.mean(np.abs(np.array([v_f4_ens_dict[i] for i in test_indices])[r_mask] - v_gt_test[r_mask])) * 3.6)
        mae_f5 = float(np.mean(np.abs(np.array([v_f5_ens_dict[i] for i in test_indices])[r_mask] - v_gt_test[r_mask])) * 3.6)
        print(f"{r_name:<18} | {mae_f0:>8.2f}k  | {mae_f1:>8.2f}k  | {mae_f2:>8.2f}k  | {mae_f4:>10.2f}k   | {mae_f5:>12.2f}k")

# ── Save Diagnostic Plots ─────────────────────────────────────────────────
os.makedirs('plots/vw4/m017_multiscale_speednet', exist_ok=True)
...
plt.title('M017: 300s GNSS Outage Position Error Comparison', fontsize=12, fontweight='bold')
plt.xlabel('Outage Elapsed Time (s)')
plt.ylabel('Position Error (m)')
plt.grid(True, alpha=0.3)
plt.legend(loc='upper left', fontsize=8)
plt.tight_layout()
plt.savefig('plots/vw4/m017_multiscale_speednet/pos_error_vs_time.png', dpi=300)
plt.close()

# Plot 2: Speed Prediction Comparison During Deceleration Zoom
zoom_slice = slice(500, 1000)
plt.figure(figsize=(12, 5))
t_zoom = time_axis[zoom_slice]
plt.plot(t_zoom, v_gt_test[zoom_slice] * 3.6, 'k--', label='Ground Truth Speed', linewidth=2)
plt.plot(t_zoom, np.array([v_w40_f4[i] for i in test_indices])[zoom_slice] * 3.6, label='F0 SpeedNet W=40 Control', alpha=0.8)
plt.plot(t_zoom, np.array([v_w20_f4[i] for i in test_indices])[zoom_slice] * 3.6, label='F1 SpeedNet W=20', alpha=0.8)
plt.plot(t_zoom, np.array([v_f4_ens_dict[i] for i in test_indices])[zoom_slice] * 3.6, label='F4 Two-Scale Ensemble', alpha=0.8)
plt.title('M017: Temporal Speed Response Comparison During Deceleration Window (t=50s to 100s)', fontsize=12, fontweight='bold')
plt.xlabel('Time (s)')
plt.ylabel('Speed (km/h)')
plt.grid(True, alpha=0.3)
plt.legend(fontsize=9)
plt.tight_layout()
plt.savefig('plots/vw4/m017_multiscale_speednet/speed_prediction_decel_zoom.png', dpi=300)
plt.close()
...
for cid, (xd, yd) in traj_series.items():
    if cid in ['F0 (Control W=40)', 'F1 (SpeedNet W=20)', 'F4 (W20+W40 Ensemble)']:
        plt.plot(xd, yd, label=f"{cid} ({nav_err_series[cid][-1]:.1f} m)")

plt.title('M017: 2D Dead-Reckoning Trajectory Comparison', fontsize=12, fontweight='bold')
...
plt.axis('equal')
plt.tight_layout()
plt.savefig('plots/vw4/m017_multiscale_speednet/trajectory_comparison.png', dpi=300)
plt.close()

# Save JSON Summary & NPZ Predictions
summary_data = {
    'milestone': 'M017',
    'title': 'Multi-Scale Temporal SpeedNet Ensemble',
    'historical_benchmark': 263.11,
    'm013_control': 254.11,
    'm014_control': 233.18,
    'selected_val_alpha': best_alpha,
    'results_table': results_table,
    'braking_diagnostics': braking_analysis_dict
}

with open('results/vw4_m017_multiscale_speednet_summary.json', 'w') as f:
    json.dump(summary_data, f, indent=2)

np.savez(
    'results/vw4_m017_multiscale_speednet_predictions.npz',
    test_indices=test_indices,
    v_gt=vbox_vel_ms[test_indices],
    v_w20=np.array([v_w20_f4[i] for i in test_indices]),
    v_w30=np.array([v_w30_f4[i] for i in test_indices]),
    v_w40=np.array([v_w40_f4[i] for i in test_indices]),
    v_ens=np.array([v_f4_ens_dict[i] for i in test_indices]),
    pos_err_f0=nav_err_series['F0 (Control W=40)'],
    pos_err_f1=nav_err_series['F1 (SpeedNet W=20)'],
    pos_err_f4=nav_err_series['F4 (W20+W40 Ensemble)']
)

print("\nSaved M017 summary JSON and predictions NPZ.")
print("M017 execution complete.")
